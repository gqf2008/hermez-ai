...
impl PluginManager {
    pub fn new() -> Self {
        let plugins_dir = hermes_core::get_hermes_home().join("plugins");
        Self {
            plugins_dir,
        ...
        for plugin in &plugins {
            ...
            for tool_name in &plugin.manifest.provides_tools {
                if let Some(ref rt) = component_runtime {
                    ...
                    let handler = Arc::new(move |args: serde_json::Value| -> hermes_core::Result<String> {
                        let args_str = args.to_string();
                        comp_rt.invoke_tool(&comp_name, &args_str)
                            .map_err(|e| hermes_core::HermesError::with_source(
...
impl hermez::plugin::host::Host for ComponentHostState {
    ...
    fn get_config(&mut self, key: String) -> Option<String> {
        // TODO: integrate with Hermes config system (~/.hermes/config.yaml)
