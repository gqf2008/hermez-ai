...
impl AIAgent {
    ...
    pub fn save_trajectory(
        ...
        let trajectories_dir = hermes_core::get_hermes_home().join("trajectories");
    ...
    /// Writes a JSON summary of the session to `~/.hermes/logs/`.
    pub fn save_session_log(&self, session_id: &str) {
        let logs_dir = hermes_core::get_hermes_home().join("logs");
