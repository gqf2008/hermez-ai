...
use hermez_core::hermez_home::{get_hermez_home, display_hermez_home};
...
/// Returns the path to `~/.hermez/config.yaml`.
pub fn get_config_path() -> PathBuf {
    get_hermez_home().join("config.yaml")
}

/// Returns the path to `~/.hermez/skills/`.
pub fn get_skills_dir() -> PathBuf {
    get_hermez_home().join("skills")
}
...
///   resolves from `HERMEZ_PLATFORM` or `HERMEZ_SESSION_PLATFORM` env vars.
...
pub fn get_disabled_skill_names(platform: Option<&str>) -> HashSet<String> {
    ...
    let Ok(content) = std::fs::read_to_string(&config_path) else {
        tracing::debug!("Could not read skill config {}", display_hermez_home());
        return HashSet::new();
    };
    ...
    let hermez_platform = std::env::var("HERMEZ_PLATFORM").ok();
    let session_platform = std::env::var("HERMEZ_SESSION_PLATFORM").ok();
    let resolved_platform = platform
        .or(hermez_platform.as_deref())
...
pub fn extract_skill_config_vars(
    ...
    let Some(hermez) = metadata.get("hermez").and_then(|v| v.as_object()) else {
        return vec![];
    };

    let Some(raw) = hermez.get("config") else {
        return vec![];
    };
