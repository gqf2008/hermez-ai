...
//! Manages:
//! - System prompt assembly (via hermez_prompt)
...
pub struct AIAgent {
    ...
    context_engine: Option<Box<dyn hermez_prompt::ContextEngine>>,
    ...
    session_db: Option<Arc<hermez_state::SessionDB>>,
...
impl AIAgent {
    ...
    pub fn with_depth(config: AgentConfig, tool_registry: Arc<ToolRegistry>, depth: u32) -> Result<Self> {
        ...
        let global_config = HermezConfig::load().ok();
        ...
        let context_engine = if config.compression_enabled {
            ...
            hermez_prompt::create_engine(engine_name, Some(comp_config))
        } else {
            None
        };
        ...
        loop {
            ...
            match llm_result {
                ...
                Ok(Err(e)) => {
                    ...
                    let classification = hermez_llm::error_classifier::classify_api_error(
    ...
    ///
    /// Dispatches to hermez_llm::client::call_llm based on the model prefix.
    async fn call_llm(
        &self,
        system_prompt: &str,
        messages: &[Message],
        ...
        let response = hermez_llm::client::call_llm(request).await
            .map_err(|e| hermez_core::HermezError::new(
                hermez_core::ErrorCategory::ApiError,
                e.to_string(),
            ))?;
    ...
    async fn call_llm_stream(
        ...
        let mut stream = hermez_llm::client::call_llm_stream(request).await
            .map_err(|e| hermez_core::HermezError::new(
                hermez_core::ErrorCategory::ApiError,
                e.to_string(),
            ))?;
        ...
        let mut final_usage: Option<hermez_llm::client::UsageInfo> = None;

        while let Some(event) = stream.next().await {
            match event {
                hermez_llm::client::LlmStreamEvent::TextDelta { delta } => {
                    if !delta.is_empty() {
                        self.fire_stream_delta(&delta);
                        text_parts.push(delta);
                    }
                }
                hermez_llm::client::LlmStreamEvent::ReasoningDelta { delta } => {
                    if !delta.is_empty() {
                        self.fire_reasoning_delta(&delta);
                        reasoning_parts.push(delta);
                    }
                }
                hermez_llm::client::LlmStreamEvent::ToolGenStarted { name } => {
                    self.fire_tool_gen_started(&name);
                }
                hermez_llm::client::LlmStreamEvent::ToolCall { id, name, arguments } => {
                    self.fire_tool_gen_started(&name);
                ...
                hermez_llm::client::LlmStreamEvent::Done { usage, .. } => {
                    final_usage = usage;
                    break;
                }
                hermez_llm::client::LlmStreamEvent::Error { message } => {
    ...
    fn has_content_after_think_block(content: &str) -> bool {
        ...
        !hermez_core::strip_think_blocks(content).trim().is_empty()
    }
    ...
    fn looks_like_codex_intermediate_ack(
        ...
        let assistant_text = hermez_core::strip_think_blocks(assistant_content);
    ...
    fn interim_content_was_streamed(&self, content: &str) -> bool {
        let visible = Self::normalize_interim_visible_text(&hermez_core::strip_think_blocks(content));
        if visible.is_empty() {
            return false;
        }
        let streamed = {
            let tracked = self.current_streamed_assistant_text.lock();
            Self::normalize_interim_visible_text(&hermez_core::strip_think_blocks(&tracked))
        };
    ...
    fn emit_interim_assistant_message(&self, assistant_msg: &Value) {
        ...
        let visible = hermez_core::strip_think_blocks(content);
...
mod tests {
    ...
    fn test_build_system_prompt() {
        ...
        assert!(prompt.contains("Hermez Agent") || prompt.contains("You are"));
    }
    ...
    fn test_stale_call_timeout_default() {
        ...
        std::env::remove_var("HERMEZ_API_CALL_STALE_TIMEOUT");
    ...
    fn test_stale_call_timeout_env_override() {
        let _guard = STALE_TIMEOUT_LOCK.lock().unwrap();
        std::env::set_var("HERMEZ_API_CALL_STALE_TIMEOUT", "60");
        let timeout = stale_call_timeout(None, &[]);
