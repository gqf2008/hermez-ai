...
//! ## How it works
//!
//! 1. `scan_skill_commands()` walks `~/.hermez/skills/` (plus external skill
...
use hermez_core::get_hermez_home;
...
pub struct SkillCommand {
    ...
    /// Embedded Hermez config overrides from frontmatter.
    pub hermez_config: Option<serde_json::Value>,
}
...
struct FrontmatterRaw {
    ...
    /// `hermez` is a catch-all for nested `hermez.config` etc.
    hermez: Option<serde_json::Value>,
}
...
///
/// Returns `~/.hermez/plans/<timestamp>-<slug>.md`.
///
...
/// Scan `~/.hermez/skills/` for SKILL.md files and populate the cache.
///
...
pub fn scan_skill_commands() -> Result<HashMap<String, SkillCommand>> {
    ...
    let skills_dir = get_hermez_home().join("skills");
    ...
    let (frontmatter, body) = match parse_frontmatter(&content) {
        ...
        // Extract hermez.config if present.
        let hermez_config = frontmatter
            .hermez
            .as_ref()
...
pub fn load_skill_payload(slug: &str) -> Result<Option<SkillPayload>> {
    ...
    Ok(Some(SkillPayload {
        name,
        content: body,
        config: skill_info.hermez_config.clone(),
...
pub fn build_skill_invocation_message(
    ...
    if let Some(config) = &skill_info.hermez_config {
...
fn format_skill_config_block(config: &serde_json::Value) -> String {
    let mut lines = vec![
        String::new(),
        "[Skill config (from ~/.hermez/config.yaml):".to_string(),
    ];
