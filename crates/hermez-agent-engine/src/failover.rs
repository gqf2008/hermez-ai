...
use hermez_llm::error_classifier::{ClassifiedError, FailoverReason};
    ...
    for msg in messages.iter_mut() {
        ...
        if let Some(obj) = value.as_object_mut() {
            ...
            if let Some(content) = obj.get("content").and_then(Value::as_str) {
                let cleaned = hermez_core::strip_think_blocks(content);
...
mod tests {
    ...
    use super::*;
    use hermez_llm::error_classifier::classify_api_error;
    ...
    fn test_strip_inline_reasoning_tags() {
        ...
        let result = hermez_core::strip_think_blocks(input);
