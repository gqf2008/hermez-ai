...
//! stored in `~/.hermes/config.yaml` under the `mcp_servers` key.

use std::collections::BTreeMap;

use hermes_core::config::McpServerConfig;
...
impl McpManager {
    ...
    pub fn get_mcp_servers() -> BTreeMap<String, McpServerConfig> {
        let Ok(config) = hermes_core::HermesConfig::load() else {
            return BTreeMap::new();
        };
        // hermes_core uses HashMap, we sort it for display
        config.mcp_servers.into_iter().collect()
    }

    /// Add or update a server entry in config.yaml.
    pub fn save_mcp_server(name: &str, server_config: &McpServerConfig) -> Result<(), String> {
        let mut config = hermes_core::HermesConfig::load()
    ...
    pub fn list_servers() -> String {
        ...
        if servers.is_empty() {
            return "No MCP servers configured.\nAdd one with: hermes mcp add <name> --url <endpoint>\n".to_string();
        }
...
mod tests {
    ...
    fn test_interpolate_value() {
        std::env::set_var("HERMES_TEST_MCP_VAR", "secret123");
        let result = McpManager::interpolate_value("${HERMES_TEST_MCP_VAR}");
        assert_eq!(result, "secret123");
        let result = McpManager::interpolate_value("Bearer ${HERMES_TEST_MCP_VAR}");
