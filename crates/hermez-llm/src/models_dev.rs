...
/// Hermez provider names → models.dev provider IDs.
...
/// Reverse mapping: models.dev ID → Hermez provider ID.
#[allow(dead_code)]
...
/// Disk cache path: `~/.hermez/models_dev_cache.json`.
fn get_disk_cache_path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".hermez").join("models_dev_cache.json"))
}
...
/// Resolve a Hermez provider ID to its models.dev provider models dict.
...
pub async fn search_models_dev(
    ...
    // Build candidates: (hermez_provider, model_id, entry)
    let candidates: Vec<(String, String, serde_json::Value)> = if let Some(p) = provider {
        ...
        for (hermez_prov, mdev_prov) in PROVIDER_TO_MODELS_DEV.iter() {
            if let Some(pdata) = data
                .get(*mdev_prov)
                .and_then(|v| v.get("models"))
                .and_then(|v| v.as_object())
            {
                for (mid, mdata) in pdata {
                    cands.push((hermez_prov.to_string(), mid.clone(), mdata.clone()));
                }
            }
        }
        cands
...
mod tests {
    ...
    fn test_provider_mapping_valid() {
        ...
        for (mdev, hermez) in MODELS_DEV_TO_PROVIDER.iter() {
            let fwd = PROVIDER_TO_MODELS_DEV.get(hermez);
            assert_eq!(fwd, Some(mdev), "reverse {} → {} has no matching forward", mdev, hermez);
        }
    }
