...
//! converts the result back into the minimal shape Hermez expects from an
...
fn resolve_command() -> String {
    std::env::var("HERMEZ_COPILOT_ACP_COMMAND")
...
fn resolve_args() -> Vec<String> {
    if let Ok(raw) = std::env::var("HERMEZ_COPILOT_ACP_ARGS") {
    ...
    pub async fn run_prompt(
        ...
        let init_req = JsonRpcRequest {
            ...
            params: serde_json::json!({
                ...
                "capabilities": {},
                "clientInfo": { "name": "hermez", "version": env!("CARGO_PKG_VERSION") },
            }),
        };
...
pub fn format_messages_as_prompt(messages: &[Value], _model: Option<&str>) -> String {
    ...
    parts.push(
        "You are being used as the active ACP agent backend for Hermez. \
