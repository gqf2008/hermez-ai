...
//!
//! Mirrors Python `hermez_cli/tips.py`.
...
const TIPS: &[&str] = &[
    ...
    "/title \"my project\" names your session — resume it later with /resume or hermez -c.",
    ...
    "Ctrl+Z suspends Hermez to the background — run fg in your shell to resume.",
    ...
    // CLI Flags
    "hermez -c resumes your most recent CLI session. hermez -c \"project name\" resumes by title.",
    "hermez -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "hermez -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "hermez chat -t web,terminal enables only specific toolsets for a focused session.",
    "hermez chat -s github-pr-workflow preloads a skill at launch.",
    "hermez chat -q \"query\" runs a single non-interactive query and exits.",
    "hermez chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "hermez chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hermez --yolo bypasses all dangerous command approval prompts for the entire session.",
    "hermez chat --source telegram tags the session for filtering in hermez sessions list.",
    "hermez -p work chat runs under a specific profile without changing your default.",

    // CLI Subcommands
    "hermez doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "hermez dump saves a compressed archive of your session for sharing or debugging.",
    "hermez sessions list shows all past sessions — filter with --since, --platform, or --query.",
    "hermez sessions export 42 > session.json exports a single session to JSON.",
    "hermez skills list shows installed skills. hermez skills install <url> adds a new one.",
    "hermez tools list shows all available tools and their allowlist status.",
    "hermez tools approve terminal permanently allows the terminal tool without prompting.",
    "hermez auth add openrouter --key <key> adds a credential to the rotation pool.",
    "hermez auth list shows all pooled credentials with masked keys.",
    "hermez auth reset openrouter clears exhaustion flags for a provider's credentials.",
    "hermez profile create work creates an isolated profile with its own config and history.",
    "hermez profile export work > work.zip exports a profile for backup or transfer.",
    "hermez cron list shows scheduled jobs. hermez cron add \"0 9 * * *\" \"morning standup\" creates one.",
    "hermez gateway start launches the messaging gateway for Telegram, Discord, Slack, etc.",
    "hermez mcp list shows configured MCP servers. hermez mcp add <name> <url> registers one.",

    // Config
    "Set model.default in ~/.hermez/config.yaml to change your default model.",
    "Set model.provider to auto and Hermez will pick the best available provider at runtime.",
    ...
    "Gateway sessions are saved to SQLite and searchable with hermez sessions list.",
    ...
    "Set HERMEZ_HOME to a different directory for completely isolated agent instances.",
    "The credential pool rotates API keys on 401/429 errors — add multiple keys for reliability.",
    "Use hermez chat --quiet for scripting — only the final response is printed to stdout.",
