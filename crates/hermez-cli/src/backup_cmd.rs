#![allow(dead_code)]
//! Backup and restore commands for Hermez Agent.
//!
//! Mirrors the Python `hermez_cli/backup.py`.
...
/// Create a backup of Hermez state.
...
pub fn cmd_backup_extended(output: Option<&str>, include_sessions: bool, _quick: bool, _label: Option<&str>) -> anyhow::Result<()> {
    ...
    let backup_name = format!("hermez_backup_{timestamp}");
...
pub fn cmd_backup_list() -> anyhow::Result<()> {
    ...
    let backups: Vec<_> = std::fs::read_dir(&current_dir)?
        ...
        .filter(|entry| {
            entry.file_name().to_string_lossy().starts_with("hermez_backup_")
        })
        .collect();
    ...
    if backups.is_empty() {
        ...
        println!("  Create one with: hermez backup");
    } else {
...
// confirm() is provided by hermez_cli::confirm()
