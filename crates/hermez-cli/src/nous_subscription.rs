...
//!
//! Mirrors Python `hermez_cli/nous_subscription.py`.
...
fn _toolset_enabled(config: &serde_yaml::Value, toolset_key: &str) -> bool {
    use hermez_tools::toolsets_def::resolve_toolset;

    let default_platform_toolsets: HashMap<&str, &str> =
        [("cli", "hermez-cli")].into_iter().collect();
...
pub fn get_nous_subscription_features(
    ...
    let managed_tools_flag = hermez_tools::tool_backend_helpers::managed_nous_tools_enabled();
    ...
    let direct_openai_tts = hermez_tools::tool_backend_helpers::resolve_openai_audio_api_key().is_some();
    ...
    let managed_web_available = managed_tools_flag
        && nous_auth_present
        && hermez_tools::managed_tool_gateway::is_managed_tool_gateway_ready("firecrawl");
    let managed_image_available = managed_tools_flag
        && nous_auth_present
        && hermez_tools::managed_tool_gateway::is_managed_tool_gateway_ready("fal-queue");
    let managed_tts_available = managed_tools_flag
        && nous_auth_present
        && hermez_tools::managed_tool_gateway::is_managed_tool_gateway_ready("openai-audio");
    let managed_browser_available = managed_tools_flag
        && nous_auth_present
        && hermez_tools::managed_tool_gateway::is_managed_tool_gateway_ready("browser-use");
    let managed_modal_available = managed_tools_flag
        && nous_auth_present
        && hermez_tools::managed_tool_gateway::is_managed_tool_gateway_ready("modal");
...
pub fn get_nous_subscription_explainer_lines() -> Vec<String> {
    ...
    vec![
        ...
        "Change these later with: hermez setup tools, hermez setup terminal, or hermez status.".into(),
    ]
}
...
/// Apply provider-level Nous defaults shared by `hermez setup` and `hermez model`.
///
...
pub fn apply_nous_managed_defaults(
    ...
    if selected_toolsets.contains("tts")
        ...
        && hermez_tools::tool_backend_helpers::resolve_openai_audio_api_key().is_none()
...
mod tests {
    ...
    fn test_toolset_enabled_empty_config() {
        ...
        // hermez-cli toolset includes web_search + web_extract
