...
//!
//! Mirrors Python: hermez auth add/list/remove/reset, hermez login/logout

use std::path::PathBuf;

use console::Style;
use hermez_llm::credential_pool::{from_entries, AuthType, Credential, CredentialPool, CredentialSource};
...
pub fn cmd_auth_list(provider_filter: Option<&str>) -> anyhow::Result<()> {
    ...
    if all_entries.is_empty() {
        ...
        if provider_filter.is_some() {
            println!("  Try: hermez auth list (no filter) to see all credentials.");
        } else {
            println!("  Add one with: hermez auth add <provider> --key <api_key>");
        }
        println!();
        return Ok(());
    }
...
fn credential_store_path() -> PathBuf {
    ...
    hermez_home.join("credentials.json")
}
...
pub(crate) fn load_auth_store() -> anyhow::Result<AuthStore> {
    hermez_core::with_auth_json_read_lock(|| {
...
pub(crate) fn save_auth_store(auth_store: &mut AuthStore) -> anyhow::Result<PathBuf> {
    hermez_core::with_auth_json_write_lock(|| {
        let path = auth_store_path();
