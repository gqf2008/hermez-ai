#![allow(dead_code)]
//! Configuration management commands for the Hermez CLI.
//!
//! Mirrors the Python `hermez config` subcommand.
...
pub fn cmd_config_show(verbose: bool) -> anyhow::Result<()> {
    ...
    println!("{}", style("Hermez Configuration").bold());
    println!("{}", "-".repeat(50));
    ...
    println!(
        "HERMEZ_HOME: {}",
        style(get_hermez_home().display()).cyan()
    );
    println!();
    ...
    if let Ok(config) = load_config_yaml(&config_path) {
        ...
        println!("{}", style("No config file found. Run `hermez setup` to create one.").dim());
    }
...
pub fn cmd_config_edit() -> anyhow::Result<()> {
    ...
    if !config_path.exists() {
        println!(
            "{} No config file at {}. Run `hermez setup` first.",
            style("[!]").yellow(),
            style(config_path.display()).cyan()
        );
        return Ok(());
...
pub fn cmd_config_check() -> anyhow::Result<()> {
    ...
    println!("{}", style("Hermez Config Check").bold());
    println!("{}", "-".repeat(40));
    println!();
    ...
    if config_path.exists() {
        ...
        issues.push("Run `hermez setup` to create config.yaml".to_string());
    }
...
pub fn cmd_config_migrate() -> anyhow::Result<()> {
    ...
    if !config_path.exists() {
        println!("{}", style("No config file found. Run `hermez setup` first.").dim());
        return Ok(());
    }
...
mod tests {
    ...
    #[test]
    fn test_get_hermez_home_env() {
        std::env::set_var("HERMEZ_HOME", "/tmp/test_hermez");
        let path = get_hermez_home();
        assert_eq!(path, PathBuf::from("/tmp/test_hermez"));
