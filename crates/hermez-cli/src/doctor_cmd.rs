...
//!
//! Mirrors the Python `hermez_cli/doctor.py`.
//! Runs a battery of diagnostic checks across the entire Hermez setup.

use std::path::PathBuf;

use console::Style;
use hermez_core::hermez_home::{display_hermez_home, get_hermez_home};
...
fn check_system_info(issues: &mut Vec<String>) {
    ...
    // Hermez CLI version
    let hermez_version = env!("CARGO_PKG_VERSION");
    check_ok(&format!("Hermez: {hermez_version}"), "");
    ...
    if env.exists() {
        ...
        if let Ok(content) = std::fs::read_to_string(&env) {
            if has_provider_env_config(&content) {
                ...
                issues.push("Run 'hermez setup' to configure API keys".to_string());
            }
        }
    } else {
        check_fail(&format!("{dhh}/.env file missing"), "");
        check_info("Run 'hermez setup' to create one");
        issues.push("Run 'hermez setup' to create .env".to_string());
    }
    ...
    if cfg.exists() {
        ...
        if let Some(config) = load_config_yaml() {
            ...
            if !stale_keys.is_empty() {
                ...
                issues.push("Stale root-level provider/base_url in config.yaml — run 'hermez doctor --fix'".to_string());
            }
...
fn check_environment(_issues: &mut Vec<String>) {
    ...
    if !any_key_found {
        ...
        check_info("Run 'hermez setup' to configure your AI provider");
    }
}
...
fn check_directory_structure(_issues: &mut Vec<String>) {
    ...
    // Check HERMEZ_HOME
    if hermez_home.exists() {
        check_ok(&format!("{dhh} directory exists"), "");

        // Check writability
        let test_file = hermez_home.join(".doctor_test");
    ...
    for subdir_name in &expected_subdirs {
        let subdir_path = hermez_home.join(subdir_name);
    ...
    if soul_path.exists() {
        ...
        check_warn(
            ...
            "(create it to give Hermez a custom personality)",
        );
    }

    // Check memories directory contents
    let memories_dir = hermez_home.join("memories");
...
fn check_session_database(issues: &mut Vec<String>) {
    ...
    let hermez_home = get_hermez_home();
    let dhh = display_hermez_home();
    let db_path = hermez_home.join("sessions.db");
    ...
    // Check WAL file size
    let wal_path = hermez_home.join("sessions.db-wal");
    if wal_path.exists() {
        if let Ok(metadata) = std::fs::metadata(&wal_path) {
            ...
            if size_mb > 50.0 {
                ...
                issues.push("Large WAL file — run 'hermez doctor --fix' to checkpoint".to_string());
...
fn check_skills_hub(_issues: &mut Vec<String>) {
    ...
    let hermez_home = get_hermez_home();
    let dhh = display_hermez_home();
    let hub_dir = hermez_home.join("skills").join(".hub");
    ...
    if hub_dir.exists() {
        ...
        check_warn("Skills Hub directory not initialized", "(run: hermez skills list)");
    }
...
fn check_cron_jobs() {
    ...
    let hermez_home = get_hermez_home();
    let cron_path = hermez_home.join("cron_jobs.json");
...
pub fn cmd_doctor_fix() -> anyhow::Result<()> {
    ...
    println!("{}", bold().apply_to("│              Hermez Doctor — Auto-Fix                  │"));
    println!("{}", bold().apply_to("└─────────────────────────────────────────────────────────┘"));

    let hermez_home = get_hermez_home();

    // Ensure HERMEZ_HOME exists
    if !hermez_home.exists() {
        print!("  {} Creating HERMEZ_HOME... ", yellow().apply_to("→"));
        match std::fs::create_dir_all(&hermez_home) {
            Ok(()) => { println!("{}", green().apply_to("✓")); fixed_count += 1; }
            Err(e) => { println!("{} {e}", red().apply_to("✗")); failed.push("create HERMEZ_HOME".to_string()); }
        }
    }
    ...
    if !cfg.exists() {
        ...
        let default_config = "# Hermez Agent configuration\nmodel:\n  name: anthropic/claude-opus-4.6\n";
    ...
    let soul = hermez_home.join("SOUL.md");
    if !soul.exists() {
        print!("  {} Creating SOUL.md template... ", yellow().apply_to("→"));
        let default_soul = "# SOUL.md — Custom personality for Hermez Agent\n# Edit this file to customize your agent's behavior\n\nYou are Hermez, a helpful AI assistant.\n";
...
pub fn cmd_doctor() -> anyhow::Result<()> {
    ...
    println!();
    println!("{}", bold().apply_to("┌─────────────────────────────────────────────────────────┐"));
    println!("{}", bold().apply_to("│                 Hermez Doctor                          │"));
    println!("{}", bold().apply_to("└─────────────────────────────────────────────────────────┘"));
    ...
    if issues.is_empty() {
        ...
        println!();
        println!("  {}", dim().apply_to("Tip: run 'hermez doctor --fix' to auto-fix what's possible."));
    }
    println!();
...
mod tests {
    ...
    fn test_get_hermez_home_from_env() {
        ...
        assert!(path.ends_with(".hermez") || path.to_string_lossy().contains(".hermez"));
    }
}
