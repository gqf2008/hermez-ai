...
fn skills_index_path() -> PathBuf {
    get_hermez_home().join(".skills_index.json")
}
...
pub fn cmd_skills_list(source: &str) -> anyhow::Result<()> {
    ...
    if filtered.is_empty() {
        ...
        println!("  Search available skills with: hermez skills search <query>");
        println!("  Or browse with: hermez skills browse");
    } else {
...
pub fn cmd_skills_search(query: &str, source: &str, limit: usize) -> anyhow::Result<()> {
    ...
    if results.is_empty() {
        ...
        println!();
        println!("  Install with: hermez skills install <name>");
    }
    println!();
...
pub fn cmd_skills_browse(page: usize, size: usize, source: &str) -> anyhow::Result<()> {
    ...
    if all_skills.is_empty() {
        ...
        if page < total_pages {
            println!("  Next page: hermez skills browse --page {}", page + 1);
        }
        println!();
        println!("  Install with: hermez skills install <name>");
    }
...
pub fn cmd_skills_inspect(identifier: &str) -> anyhow::Result<()> {
    ...
    if skill_path.exists() {
        ...
        if let Some(skill) = results.into_iter().find(|s| s.name == identifier) {
            ...
            println!();
            println!("  Install with: hermez skills install {}", identifier);
        } else {
...
fn skills_taps_path() -> PathBuf {
    get_hermez_home().join(".skill_taps.json")
}
...
pub fn cmd_skills_snapshot_import(path: &str) -> anyhow::Result<()> {
    ...
    println!("  {}", dim().apply_to("Run `hermez skills install <name>` for each skill to install."));
    println!();
    Ok(())
}
...
pub fn cmd_skills_config() -> anyhow::Result<()> {
    ...
    println!();
    println!("  {}", dim().apply_to("Use `hermez skills enable/disable <name>` to toggle."));
    println!();
    Ok(())
}
