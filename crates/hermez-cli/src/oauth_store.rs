...
//! - Fallback: plaintext JSON file in ~/.hermez/ (protected by 0o600 permissions on Unix)
//!
//! Service names are scoped as `hermez-agent/{provider}` to avoid collisions.
...
/// in `~/.hermez/secure_credentials/{provider}.json`.
...
pub fn list_stored_providers() -> Vec<String> {
    ...
    let dir = hermez_core::get_hermez_home().join("secure_credentials");
...
mod tests {
    ...
    fn test_service_name() {
        assert_eq!(service_name("google"), "hermez-agent/google");
    }
