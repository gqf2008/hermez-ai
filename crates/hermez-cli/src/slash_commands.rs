//! Slash command system for the Hermez interactive CLI.
//!
//! Mirrors Python `hermez_cli/commands.py` — central registry for all
...
use hermez_agent_engine::agent::{AIAgent, AgentConfig};
...
pub const COMMAND_REGISTRY: &[CommandDef] = &[
    ...
    CommandDef::new(
        "snapshot",
        "Create or restore state snapshots of Hermez config/state",
        "Session",
    )
    .with_aliases(&["snap"])
    ...
    CommandDef::new("image", "Attach a local image file for your next prompt", "Info")
        .with_args_hint("<path>")
        .cli_only(),
    CommandDef::new("update", "Update Hermez Agent to the latest version", "Info"),
...
fn cmd_toolsets(_ctx: &mut SlashContext) -> SlashResult {
    ...
    for (name, ts) in hermez_tools::toolsets_def::toolsets().iter() {
...
fn cmd_skills(_ctx: &mut SlashContext, _args: &str) -> SlashResult {
    println!("Skills management: use `hermez skills` subcommand for full management.");
    SlashResult::Handled
}

fn cmd_cron(_ctx: &mut SlashContext, _args: &str) -> SlashResult {
    println!("Cron management: use `hermez cron` subcommand for full management.");
    SlashResult::Handled
}

fn cmd_reload(_ctx: &mut SlashContext) -> SlashResult {
    // Reload .env
    let env_path = hermez_core::get_hermez_home().join(".env");
    if env_path.exists() {
...
fn cmd_insights(_ctx: &mut SlashContext, _args: &str) -> SlashResult {
    println!("Insights: use `hermez insights` subcommand for detailed analytics.");
    SlashResult::Handled
}

fn cmd_platforms(_ctx: &mut SlashContext) -> SlashResult {
    println!("Gateway platforms: use `hermez gateway` subcommand for status.");
    SlashResult::Handled
}
...
fn cmd_debug(_ctx: &mut SlashContext) -> SlashResult {
    println!("Debug report: use `hermez debug` subcommand.");
    SlashResult::Handled
}
