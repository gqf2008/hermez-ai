...
//!
//! Mirrors the Python `hermez_cli/gateway.py` service management functions.
...
const SERVICE_BASE: &str = "hermez-gateway";

fn get_service_name() -> String {
    let home = hermez_core::get_hermez_home();
    let default = dirs::home_dir().map(|d| d.join(".hermez")).unwrap_or_default();
    if home == default {
    ...
    // Fallback: just the base name (profile scoping via HERMEZ_HOME in env)
...
fn pid_file_path() -> std::path::PathBuf {
    hermez_core::get_hermez_home().join("gateway.pid")
}
...
fn hermez_exe_path() -> String {
    // Try to find the `hermez` binary
    if let Ok(path) = which::which("hermez") {
        return path.to_string_lossy().to_string();
    }
    // Fallback to current exe
    std::env::current_exe()
        .map(|p| p.to_string_lossy().to_string())
        .unwrap_or_else(|_| "hermez".to_string())
}
...
fn install_systemd_service() -> Result<(), String> {
    ...
    let exe = hermez_exe_path();
    let hermez_home = hermez_core::get_hermez_home();

    let unit_content = format!(
        "[Unit]\n\
Description=Hermez Agent Gateway - Messaging Platform Integration\n\
...
<dict>
    ...
    <string>{hermez_home}/logs/gateway.stdout.log</string>
    <key>StandardErrorPath</key>
    <string>{hermez_home}/logs/gateway.stderr.log</string>
</dict>
</plist>
"#,
        hermez_home = hermez_home.display(),
    );
...
fn start_windows_background() -> Result<(), String> {
    let exe = hermez_exe_path();
    let hermez_home = hermez_core::get_hermez_home();
    let log_dir = hermez_home.join("logs");
    ...
    cmd.arg("gateway");
    cmd.env("HERMEZ_HOME", &hermez_home);
...
pub fn cmd_gateway_status(_deep: bool, _system: bool) -> Result<(), String> {
    ...
    println!("  HERMEZ_HOME: {}", hermez_core::get_hermez_home().display());
    println!();

    // Show configured platforms
    let hermez_home = hermez_core::get_hermez_home();
    let config_path = hermez_home.join("config.yaml");
    if config_path.exists() {
        if let Ok(content) = std::fs::read_to_string(&config_path) {
            ...
            if let Some(platforms) = config.get("gateway").and_then(|g| g.get("platforms")).and_then(|p| p.as_sequence()) {
                ...
                println!("    or add platforms to ~/.hermez/config.yaml");
            }
        }
    }
...
pub fn cmd_gateway_install(_force: bool, _system: bool, _run_as_user: Option<&str>) -> Result<(), String> {
    ...
    if has_systemd() {
        ...
        println!("  {} Windows: use 'hermez gateway start' to run in background", yellow.apply_to("→"));
...
pub fn cmd_gateway_migrate_legacy() -> Result<(), String> {
    ...
    let hermez_home = hermez_core::get_hermez_home();
    let old_config = hermez_home.join("gateway.yaml");
    let new_config = hermez_home.join("gateway_config.yaml");
    ...
    println!();
    println!("  {}", dim.apply_to("Review the new file and run `hermez gateway setup` to adjust platforms if needed."));
    println!();
...
pub fn cmd_gateway_setup() -> Result<(), String> {
    ...
    println!();
    println!("  {}", Style::new().dim().apply_to("Run `hermez gateway start` to launch after configuration."));
    println!("  Platform credentials are stored in ~/.hermez/gateway_config.yaml");
    println!();
