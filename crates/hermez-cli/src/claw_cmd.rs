#![allow(dead_code)]
//! Claw migration command — migrate from OpenClaw (or Claude Code) to Hermez.
//!
//! Mirrors Python: `hermez claw` in `hermez_cli/claw.py`.
//!
//! For OpenClaw migration, the Python script `openclaw_to_hermez.py` is the
...
fn migration_script_candidates() -> Vec<PathBuf> {
    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(parent) = exe.parent() {
            ...
            candidates.push(parent.join("optional-skills/migration/openclaw-migration/scripts/openclaw_to_hermez.py"));
            // Try parent of parent.
            if let Some(pp) = parent.parent() {
                candidates.push(pp.join("optional-skills/migration/openclaw-migration/scripts/openclaw_to_hermez.py"));
                // Try going up more levels (common in dev setups: target/release/hermes).
                if let Some(ppp) = pp.parent() {
                    candidates.push(ppp.join("optional-skills/migration/openclaw-migration/scripts/openclaw_to_hermez.py"));
                    // Project root may be another level up (e.g. hermez-rs/ under project root).
                    if let Some(pppp) = ppp.parent() {
                        candidates.push(pppp.join("optional-skills/migration/openclaw-migration/scripts/openclaw_to_hermez.py"));
                    }
                }
            }
        }
    ...
    // HERMEZ_HOME skills directory (user installed from Skills Hub).
    if let Ok(home) = std::env::var("HERMEZ_HOME") {
        candidates.push(PathBuf::from(&home).join("skills/migration/openclaw-migration/scripts/openclaw_to_hermez.py"));
    } else if let Some(dir) = dirs::home_dir() {
        candidates.push(dir.join(".hermez/skills/migration/openclaw-migration/scripts/openclaw_to_hermez.py"));
    }

    // Current working directory (dev setup).
    candidates.push(PathBuf::from("optional-skills/migration/openclaw-migration/scripts/openclaw_to_hermez.py"));
    candidates.push(PathBuf::from("../optional-skills/migration/openclaw-migration/scripts/openclaw_to_hermez.py"));
...
fn migrate_openclaw(
    ...
    let Some(source_dir) = find_openclaw_dir(source) else {
        ...
        println!("  {}", dim().apply_to("Specify a custom path: hermez claw migrate --source /path/to/.openclaw"));
        return Ok(());
    };
    ...
    let Some(script_path) = find_migration_script() else {
        ...
        println!("  {}", dim().apply_to("Install the openclaw-migration skill: hermez skills install openclaw-migration"));
        return Ok(());
    };
    ...
    let Some(python) = find_python() else {
        ...
        println!("  {}", dim().apply_to("Install Python and re-run, or use the Python Hermez CLI."));
        return Ok(());
    };
    ...
    println!();
    println!("{}", cyan().apply_to("◆ Hermez — OpenClaw Migration"));
    println!();
    println!("  Source:      {}", source_dir.display());
    println!("  Target:      {}", hermez_home.display());
    ...
    let preview_status = run_python_script(
        &python, &script_path, &source_dir, &hermez_home,
    ...
    println!("  {}", dim().apply_to("Run `hermez claw cleanup` to archive leftover OpenClaw directories."));
    println!();
...
fn run_python_script(
    ...
    cmd.arg("--target").arg(hermez_home);
...
fn migrate_claude_code(force: bool) -> anyhow::Result<()> {
    ...
    let hermez_home = get_hermez_home();
    std::fs::create_dir_all(&hermez_home)?;
    ...
    if claude_settings.exists() {
        if let Ok(content) = std::fs::read_to_string(&claude_settings) {
            if let Ok(settings) = serde_json::from_str::<serde_json::Value>(&content) {
                let config_file = hermez_home.join("config.yaml");
    ...
    if claude_memories.exists() {
        ...
        let hermez_memories = hermez_home.join("memories");
        std::fs::create_dir_all(&hermez_memories)?;

        match std::fs::read_dir(&claude_memories) {
            Ok(entries) => {
                for entry in entries.flatten() {
                    let src = entry.path();
                    let dst = hermez_memories.join(entry.file_name());
                    if src.is_file() {
                        if dst.exists() && !force {
                            skipped += 1;
    ...
    println!("  {}", dim().apply_to("Run `hermez claw cleanup` to remove migration artifacts."));
    println!();
...
fn cmd_cleanup(source: Option<&str>) -> anyhow::Result<()> {
    ...
    let hermez_home = get_hermez_home();
    let temp_dir = hermez_home.join(".migration");
    if temp_dir.exists() {
...
pub fn cmd_claw(
    ...
    match action {
        ...
        _ => {
            println!("Usage: hermez claw <command> [options]");
            println!();
            println!("Commands:");
            println!("  migrate          Migrate settings from OpenClaw or Claude Code to Hermez");
            println!("  cleanup          Archive leftover OpenClaw directories after migration");
            println!();
            println!("Run 'hermez claw <command> --help' for options.");
            Ok(())
        }
    }
}
...
mod tests {
    ...
    fn test_migration_script_candidates_non_empty() {
        ...
        assert!(candidates.iter().any(|p| p.to_string_lossy().contains("openclaw_to_hermez.py")));
    }
