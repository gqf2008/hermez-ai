#![allow(dead_code)]
//! Interactive setup wizard for Hermez Agent.
//!
//! Mirrors the Python `hermez_cli/setup.py`.
...
fn setup_gateway(config: &mut serde_yaml::Value) -> io::Result<()> {
    ...
    println!("\n{}", dim().apply_to("Connect Hermez to messaging platforms."));
    ...
    if !selected.is_empty() {
        ...
        println!("  You can run `hermez setup gateway` later to add platforms.");
    }
...
fn run_quick_setup(config: &mut serde_yaml::Value) -> io::Result<()> {
    ...
    println!("\n  {} Quick setup complete. Run `hermez setup` for full configuration.", green().apply_to("✓"));
    Ok(())
}
...
pub fn cmd_setup() -> anyhow::Result<()> {
    ...
    println!("{}", bold().apply_to("║     Hermez Agent Setup Wizard       ║"));
    println!("{}", bold().apply_to("╚══════════════════════════════════════╝"));
    println!("\nHERMEZ_HOME: {}", cyan().apply_to(home.display()));
    ...
    if is_first_install {
        ...
        if idx == 0 {
            ...
            save_config(&config)?;
            println!("\n  {} Done! Run `hermez` to start chatting.", green().apply_to("✓"));
            return Ok(());
        }
    }
    ...
    println!("\nStart chatting with: {}", green().apply_to("hermez"));
...
pub fn cmd_setup_reset() -> anyhow::Result<()> {
    ...
    for file in ["config.yaml", ".env", "SOUL.md"] {
        let path = get_hermez_home().join(file);
        if path.exists() {
