...
//! Self-update command.
//!
//! Mirrors Python: hermez update (self-update from PyPI or Git)
...
pub fn cmd_update(preview: bool, force: bool, _gateway: bool) -> anyhow::Result<()> {
    println!();
    println!("{}", cyan().apply_to("◆ Hermez Update"));
    println!();
    ...
    match check_for_updates(preview) {
        ...
        Err(e) => {
            ...
            println!("  {}", dim().apply_to("Update manually via: cargo install hermez --locked"));
            println!("  {}", dim().apply_to("Or: pip install --upgrade hermez-agent"));
        }
    }
    println!();
...
fn check_for_updates(_preview: bool) -> anyhow::Result<Option<String>> {
    ...
    let urls = [
        ("GitHub", "https://api.github.com/repos/nousresearch/hermez-agent/releases/latest", "tag_name"),
        ("PyPI", "https://pypi.org/pypi/hermez-agent/json", "version"),
    ];
...
fn apply_update(_version: &str) -> anyhow::Result<()> {
    ...
    let output = std::process::Command::new("pip")
        .args(["install", "--upgrade", "hermez-agent"])
        .output();
