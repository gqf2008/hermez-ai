...
//!
//! Mirrors Python: hermez model (interactive TUI for selecting/viewing models)
...
pub fn cmd_model() -> anyhow::Result<()> {
    ...
    println!();
    println!("  {}", dim().apply_to("Configure with: hermez config set model.provider <provider>"));
    println!("  {}", dim().apply_to("Configure with: hermez config set model.name <model>"));
    println!();
...
pub fn cmd_model_switch(model: &str) -> anyhow::Result<()> {
    ...
    // Load current config
    let mut config = hermez_core::HermezConfig::load().unwrap_or_default();
...
pub fn cmd_model_info(model: &str) -> anyhow::Result<()> {
    ...
    let context_length = hermez_llm::model_metadata::get_model_context_length(
        &resolved,
        "",  // base_url
