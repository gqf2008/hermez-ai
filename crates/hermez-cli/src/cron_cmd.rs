...
//!
//! Mirrors Python: hermez cron list/create/pause/resume/delete/edit/run/status/tick

use console::Style;

use hermez_cron::jobs::{JobStore, JobUpdates};
...
pub fn cmd_cron_list(all: bool) -> anyhow::Result<()> {
    ...
    let store = match get_store() {
        ...
        None => {
            ...
            println!("  Create one with: hermez cron create");
            println!();
            return Ok(());
        }
    };
...
pub fn cmd_cron_create(
    ...
    println!();
    println!("  Start the scheduler with: hermez cron start");
    println!();
...
pub fn cmd_cron_status() -> anyhow::Result<()> {
    ...
    println!("  Start the gateway with: hermez gateway start");
    println!();
