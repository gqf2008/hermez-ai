...
//!
//! Mirrors the Python `hermez_cli/profiles.py` module.
//! Each profile is an isolated HERMEZ_HOME directory with its own config,
...
use console::Style;
use hermez_core::{get_default_hermez_root, get_hermez_home};
...
const RESERVED_NAMES: &[&str] = &["hermez", "default", "test", "tmp", "root", "sudo"];

/// Hermez subcommands that cannot be used as profile names/aliases.
const HERMEZ_SUBCOMMANDS: &[&str] = &[
...
fn get_profiles_root() -> PathBuf {
    get_default_hermez_root().join("profiles")
}

/// Return the path to the sticky active_profile file.
fn active_profile_path() -> PathBuf {
    get_default_hermez_root().join("active_profile")
}
...
fn check_alias_collision(name: &str) -> Option<String> {
    ...
    if HERMEZ_SUBCOMMANDS.contains(&name) {
        return Some(format!("'{name}' conflicts with a hermez subcommand"));
    }
...
pub fn list_profiles() -> Vec<ProfileInfo> {
    ...
    // Default profile
    let default_home = get_hermez_home();
...
pub fn cmd_profile_list() -> anyhow::Result<()> {
    ...
    let default_home = get_hermez_home();
    let profiles_root = get_profiles_root();
    println!("  HERMEZ_HOME: {}", display_path(&default_home));
...
pub fn cmd_profile_create(
    ...
    println!(
        "  {}",
        cyan.apply_to("Set HERMEZ_HOME to switch profiles:")
    );
    println!(
        "    {}",
        cyan.apply_to(format!("  HERMEZ_HOME={} hermez", profile_dir.display()))
    );
    println!();
...
pub fn cmd_profile_show(name: &str) -> anyhow::Result<()> {
    ...
    if name == "default" {
        ...
        println!("  HERMEZ_HOME: {}", display_path(&home));
        println!();

        if home.join("config.yaml").exists() {
            println!("  Config:  present");
        } else {
            println!("  {}", dim.apply_to("Config: not found — run 'hermez setup' to configure"));
        }
    ...
    if !profile_dir.exists() {
        println!(
            "  {} Profile '{name}' not found. Create it with: hermez profile create {name}",
...
pub fn cmd_profile_export(name: &str, output: Option<&str>) -> anyhow::Result<()> {
    ...
    let profiles_root = get_profiles_root();
    let default_root = get_default_hermez_root();
...
fn export_default_profile(root: &Path, out_path: &str) -> io::Result<std::process::Output> {
    ...
    let exclude_root: &[&str] = &[
        "hermez-agent", ".worktrees", "profiles", "bin", "node_modules",
        "state.db", "state.db-shm", "state.db-wal", "hermez_state.db",
        "response_store.db", "response_store.db-shm", "response_store.db-wal",
        "gateway.pid", "gateway_state.json", "processes.json",
        "auth.json", ".env", "auth.lock", "active_profile",
        ".update_check", "errors.log", ".hermez_history",
...
/// Switch to a profile (sets HERMEZ_HOME).
pub fn cmd_profile_use(name: &str) -> anyhow::Result<()> {
    ...
    if !profile_dir.exists() {
        println!(
            "  {} Profile '{name}' not found. Create it first with: hermez profile create {name}",
            yellow.apply_to("\u{2717}")
        );
        return Ok(());
    }
    ...
    println!(
        "    {}",
        green.apply_to(format!("HERMEZ_HOME={}", profile_dir.display()))
    );
    println!();
...
You are Hermez, a helpful, capable AI assistant. You have access to tools
...
mod tests {
    ...
    fn test_check_alias_collision_reserved() {
        ...
        assert!(check_alias_collision("hermez").is_some());
    ...
    fn test_display_path_home() {
        if let Some(home) = dirs::home_dir() {
            let path = home.join(".hermez/profiles/test");
