#![allow(dead_code)]
//! Dump command for Hermez CLI.
//!
//! Outputs a compact, plain-text summary of the user's Hermez setup
//! that can be copy-pasted into Discord/GitHub/Telegram for support context.
//! No ANSI colors, no checkmarks — just data.
//!
//! Mirrors the Python `hermez_cli/dump.py`.

use std::collections::HashMap;
use std::fs;
use std::path::Path;

use hermez_core::config::HermezConfig;
...
fn count_skills(hermez_home: &Path) -> usize {
...
fn count_mcp_servers(config: &HermezConfig) -> usize {
    config.mcp_servers.len()
}

/// Return cron jobs summary from cron_jobs.json.
fn cron_summary(hermez_home: &Path) -> String {
    let jobs_file = hermez_home.join("cron_jobs.json");
    if !jobs_file.exists() {
        // Also check cron/jobs.json (newer path)
        let alt_file = hermez_home.join("cron").join("jobs.json");
        if alt_file.exists() {
...
fn memory_provider(config: &HermezConfig) -> &str {
    config.memory.backend.as_deref().unwrap_or("built-in")
}

/// Extract model and provider from config.
fn get_model_and_provider(config: &HermezConfig) -> (&str, &str) {
...
fn config_overrides(config: &HermezConfig) -> HashMap<String, String> {
    let defaults = HermezConfig::default();
...
fn get_active_profile_name() -> String {
    // Check HERMEZ_PROFILE env var or detect from HERMEZ_HOME path
    if let Ok(profile) = std::env::var("HERMEZ_PROFILE") {
        if !profile.is_empty() {
            return profile;
        }
    }
    // If HERMEZ_HOME contains "/profiles/", extract the profile name
...
pub fn cmd_dump(show_keys: bool) -> anyhow::Result<()> {
    ...
    let config = HermezConfig::load().unwrap_or_default();
    ...
    lines.push("--- hermez dump ---".to_string());
    ...
    lines.push(format!("hermez_home:      {}", display_hermez_home()));
    ...
    lines.push(format!("  cron_jobs:          {}", cron_summary(&hermez_home)));
    lines.push(format!("  skills:             {}", count_skills(&hermez_home)));
