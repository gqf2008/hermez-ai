...
//!
//! Mirrors the Python `environments/hermez_swe_env.py` CLI interface.
//!
//! Provides:
//! - `hermez swe evaluate` — run SWE evaluation on a dataset
//! - `hermez swe benchmark` — run built-in benchmark suite
//! - `hermez swe env` — show environment info and test sandbox

use std::path::Path;
use std::sync::Arc;

use console::Style;
use hermez_rl::Environment;
use hermez_rl::base::{AgentRunner, AgentResult, Message as RlMessage};
...
struct SweAgentRunner {
    agent: tokio::sync::Mutex<hermez_agent_engine::AIAgent>,
...
impl AgentRunner for SweAgentRunner {
    async fn run(&self, messages: Vec<RlMessage>) -> AgentResult {
        ...
        AgentResult {
            ...
            turns_used,
            finished_naturally: turn_result.exit_reason == hermez_agent_engine::agent::ExitReason::NaturalStop,
...
pub async fn cmd_swe_evaluate(opts: &SweEvaluateOptions) -> anyhow::Result<()> {
    ...
    let sandbox_backend = match opts.sandbox.as_str() {
        "docker" => hermez_rl::swe_env::SandboxBackend::Docker,
        _ => hermez_rl::swe_env::SandboxBackend::Local,
    };

    // Build config
    let mut config = hermez_rl::swe_env::SweEnvConfig::default();
    ...
    // Setup environment
    let mut env = hermez_rl::swe_env::SweEnv::with_config(config);

    // Attach real agent if requested
    if opts.use_agent {
        let mut agent_config = hermez_agent_engine::AgentConfig::default();
        if let Some(ref model) = opts.model {
            agent_config.model = model.clone();
        }
        agent_config.max_iterations = opts.quick.then_some(15).unwrap_or(30);
        let tool_registry = Arc::new(hermez_tools::registry::ToolRegistry::new());
        match hermez_agent_engine::AIAgent::new(agent_config, tool_registry) {
            Ok(agent) => {
    ...
    for (i, item) in env.eval_items[..eval_size].iter().enumerate() {
        ...
        let mut messages = vec![hermez_rl::base::Message::user(&prompt)];
        ...
        messages.push(hermez_rl::base::Message::assistant(&response));

        let result = hermez_rl::base::AgentResult::new(messages);
...
pub fn cmd_swe_env_info() -> anyhow::Result<()> {
    ...
    let builtin_tasks = hermez_rl::swe_env::builtin_swe_tasks();
