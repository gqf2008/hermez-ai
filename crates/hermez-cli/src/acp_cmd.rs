...
//!
//! Mirrors Python: hermez acp (IDE integration for VS Code, Zed, JetBrains)
...
pub fn cmd_acp(action: &str, editor: Option<&str>) -> anyhow::Result<()> {
    ...
    match action {
        "status" => {
            ...
            println!();
            println!("  {}", dim().apply_to("Run `hermez acp install <editor>` to set up IDE integration."));
        }
        "install" => {
            ...
            match ed.to_lowercase().as_str() {
                "vscode" | "vs-code" | "vs_code" => {
                    ...
                    println!("  {}", dim().apply_to("Install the Hermez extension from the VS Code Marketplace."));
                    println!("  {}", dim().apply_to("Search for 'Hermez Agent' in the Extensions view."));
                }
                "zed" => {
                ...
                "jetbrains" | "idea" | "webstorm" | "pycharm" => {
                    ...
                    println!("  {}", dim().apply_to("Install the Hermez plugin from the JetBrains Plugin Marketplace."));
                }
                _ => {
        ...
        "run" => {
            // Delegate to the hermez_acp binary for the actual ACP server
            let exe = std::env::current_exe()
                .ok()
                .and_then(|p| p.parent().map(|d| d.join("hermez_acp")))
                .filter(|p| p.exists());
            ...
            if let Some(acp_path) = exe {
                ...
                println!("  {}", yellow().apply_to("⚠ hermez_acp binary not found."));
                println!("  {}", dim().apply_to("Build it with: cargo build --bin hermez_acp"));
            }
        }
        _ => {
            println!("  {}", dim().apply_to("Usage: hermez acp <status|install|run>"));
        }
    }
    println!();
