//! Reedline completers for Hermez CLI.
//!
...
/// Completer for Hermez CLI slash commands.
...
pub struct HermezCompleter {
    command: CommandCompleter,
    file: FileCompleter,
}

impl HermezCompleter {
    pub fn new() -> Self {
        Self {
...
impl Default for HermezCompleter {
    fn default() -> Self {
        Self::new()
    }
}

impl Completer for HermezCompleter {
    fn complete(&mut self, line: &str, pos: usize) -> Vec<Suggestion> {
...
mod tests {
    ...
    fn test_combined_completer() {
        let mut completer = HermezCompleter::new();
        // "/" matches commands
        let results = completer.complete("/", 1);
        assert!(!results.is_empty());
