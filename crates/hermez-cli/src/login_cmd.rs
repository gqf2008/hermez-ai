...
//! Login / OAuth flow.
//!
//! Mirrors Python: hermez login (OAuth login for supported providers)
...
async fn cmd_login_async(
    ...
    let Some(config) = config else {
        ...
        println!();
        println!("  {}", dim().apply_to("Use `hermez auth add <provider> --key <api_key>` for API key auth."));
        println!();
        return Ok(());
    };
...
pub fn cmd_oauth_list() -> anyhow::Result<()> {
    ...
    if providers.is_empty() {
        ...
        println!();
        println!("  {}", dim().apply_to("Run `hermez login <provider>` to authenticate."));
    } else {
        for p in &providers {
