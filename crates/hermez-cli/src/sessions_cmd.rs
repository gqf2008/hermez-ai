#![allow(dead_code)]
//! Session management commands for the Hermez CLI.
//!
//! Mirrors the Python `hermez sessions` subcommand.
...
pub fn cmd_sessions_prune(
    ...
    let cutoff_count = cutoff_sessions.iter()
        .filter(|sp| {
            let age_days = (hermez_state::now_epoch() - sp.session.started_at) / 86400.0;
...
mod tests {
    ...
    fn test_export_not_found() {
        let db = SessionDB::open(":memory:").unwrap();
        let tmp = std::env::temp_dir().join("hermez_test_export.jsonl");
