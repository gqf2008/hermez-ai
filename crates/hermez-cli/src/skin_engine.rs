#![allow(dead_code)]
//! Hermez CLI skin/theme engine.
//!
//! Mirrors Python `hermez_cli/skin_engine.py`.
//! Data-driven skin system that lets users customize the CLI's visual appearance.
//! Skins are defined as YAML files in ~/.hermez/skins/ or as built-in presets.
...
fn builtin_default() -> SkinConfig {
    ...
    branding.insert(
        ...
        "Hermez Agent".to_string(),
    );
    branding.insert(
        "welcome".to_string(),
        "Welcome to Hermez Agent! Type your message or /help for commands.".to_string(),
    );
    branding.insert("goodbye".to_string(), "Goodbye! ⚕".to_string());
    branding.insert("response_label".to_string(), " ⚕ Hermez ".to_string());
    ...
    SkinConfig {
        ...
        description: "Classic Hermez — gold and kawaii".to_string(),
        colors,
        branding,
...
/// Load a user-defined skin from ~/.hermez/skins/<name>.yaml.
...
/// Save a user-defined skin to ~/.hermez/skins/<name>.yaml.
...
fn active_skin_path() -> PathBuf {
    let hermez_home = hermez_core::get_hermez_home();
    hermez_home.join(".active_skin")
}

/// Get the currently active skin.
///
/// Priority:
/// 1. ~/.hermez/.active_skin file content
...
pub fn cmd_skin_list() -> anyhow::Result<()> {
    ...
    println!(
        "  {} Use `hermez skin apply <name>` to switch",
        dim.apply_to("→")
    );
    println!();
...
pub fn cmd_skin_apply(name: &str) -> anyhow::Result<()> {
    ...
    let skin = resolve_skin(name).ok_or_else(|| {
        anyhow::anyhow!(
            "Skin '{}' not found. Run `hermez skin list` to see available skins.",
            name
        )
    })?;
...
pub fn cmd_skin_preview(name: &str) -> anyhow::Result<()> {
    ...
    println!();
    println!(
        "  {} Run `hermez skin apply {}` to activate this skin.",
        yellow.apply_to("→"),
        name
    );
    println!();
...
mod tests {
    ...
    fn test_get_branding() {
        let skin = get_builtin_skin("default").unwrap();
        assert_eq!(skin.get_branding("agent_name", ""), "Hermez Agent");
