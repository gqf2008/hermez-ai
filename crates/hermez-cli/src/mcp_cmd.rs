...
fn mcp_config_path() -> PathBuf {
    get_hermez_home().join("mcp_servers.json")
}
...
pub fn cmd_mcp_list() -> anyhow::Result<()> {
    ...
    if servers.is_empty() {
        ...
        println!("  Add one with: hermez mcp add <name> --command <cmd>");
    } else {
...
pub fn cmd_mcp_configure(_name: &str) -> anyhow::Result<()> {
    ...
    println!();
    println!("  Add with: hermez mcp add <name> --command <cmd> [--args \"...\"]");
    println!();
...
pub fn cmd_mcp_serve(verbose: bool) -> anyhow::Result<()> {
    if !verbose {
        ...
        println!();
        println!("  {}", dim().apply_to("Starting Hermez MCP server over stdio..."));
        println!();
    }

    let rt = tokio::runtime::Runtime::new()?;
    rt.block_on(hermez_tools::mcp_serve::run_mcp_server(verbose))
