...
//! Mirrors the Python `hermes_cli/banner.py`.
...
/// Hermes Agent logo in ASCII block art.
/// Styled with gold/bronze gradients.
pub const HERMES_AGENT_LOGO: &str = r#"
    ██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗
    ██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝
    ███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║
    ██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║
...
pub const HERMES_CADUCEUS: &str = r#"
              ⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀
    ⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀
  ⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀
    ⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉
...
pub fn check_for_updates() -> Option<u32> {
    ...
    let repo_dir = if repo_dir.join(".git").exists() {
        ...
        std::env::current_dir().ok()?.join("hermes-agent")
    };
...
/// Resolve the active Hermes git checkout, or None if this isn't a git install.
...
pub fn format_banner_version_label() -> String {
    ...
    let base = format!("Hermes Agent v{version}");
...
///
/// Shows the Hermes logo, model info, available tools/skills summary,
...
pub fn print_welcome_banner(
    ...
    if let Ok((width, _)) = crossterm::terminal::size() {
        if width >= 95 {
            println!();
            println!("{}", accent.apply_to(HERMES_AGENT_LOGO));
            println!();
        }
    }
    ...
    if let Some(behind) = check_for_updates() {
        if behind > 0 {
            ...
            println!(
                ...
                dim.apply_to("— run `hermes update` to update")
            );
        }
    }
...
mod tests {
    ...
    fn test_banner_version_label_without_git() {
        ...
        assert!(label.contains("Hermes Agent"));
        assert!(label.contains(env!("CARGO_PKG_VERSION")));
    }

    #[test]
    fn test_logo_not_empty() {
        assert!(!HERMES_AGENT_LOGO.is_empty());
        assert!(!HERMES_CADUCEUS.is_empty());
    }
