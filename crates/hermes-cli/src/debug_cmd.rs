...
/// Print debug info about the current Hermes installation.
pub fn cmd_debug() -> anyhow::Result<()> {
    let home = get_hermes_home();

    println!();
    println!("{}", cyan().apply_to("◆ Hermes Debug Info"));
    println!();
    ...
    if config_path.exists() {
        ...
        println!();
        println!("  {}", yellow().apply_to("No config.yaml found. Run `hermes setup` to configure."));
    }
...
pub fn cmd_dump_session(session_id: &str, _show_keys: bool) -> anyhow::Result<()> {
    ...
    let db = hermes_state::SessionDB::open(&db_path)?;
...
pub fn cmd_dump_all(show_keys: bool) -> anyhow::Result<()> {
    ...
    println!();
    println!("{}", cyan().apply_to("◆ Full Hermes Dump"));
    println!();
    println!("  HERMES_HOME: {}", home.display());
    println!();
