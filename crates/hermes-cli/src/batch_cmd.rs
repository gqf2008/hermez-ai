...
use hermes_batch::{BatchConfig, BatchRunner, list_distributions};
...
pub fn cmd_batch_run(opts: &BatchRunOptions) -> anyhow::Result<()> {
    ...
    if let Some(ref dist) = opts.distribution {
        if !hermes_batch::validate_distribution(dist) {
    ...
    let rt = tokio::runtime::Runtime::new().map_err(|e| {
        hermes_core::HermesError::new(hermes_core::ErrorCategory::InternalError, e.to_string())
    })?;
...
pub fn cmd_batch_distributions() -> anyhow::Result<()> {
    ...
    println!();
    println!("  Use --distribution <name> with 'hermes batch run' to sample toolsets.");
    println!();
...
pub fn cmd_batch_status(run_name: &str) -> anyhow::Result<()> {
    ...
    if !checkpoint_path.exists() {
        ...
        println!("    {}", dim.apply_to("Start with: hermes batch run <dataset.jsonl> --name <run_name>"));
        println!();
        return Ok(());
    }

    let checkpoint = hermes_batch::Checkpoint::load(&checkpoint_path)
        .ok()
        .flatten();
