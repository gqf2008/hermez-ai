...
//!
//! Mirrors Python: hermes whatsapp (configure WhatsApp Cloud API)
...
pub fn cmd_whatsapp(action: &str, token: Option<&str>, phone_id: Option<&str>) -> anyhow::Result<()> {
    ...
    match action {
        "setup" => {
            ...
            println!();
            println!("  {}", dim().apply_to("Then run: hermes whatsapp connect --token <token> --phone-id <id>"));
        }
        "connect" => {
            ...
            let config_path = get_hermes_home().join("gateway_config.yaml");
            ...
            println!("  {}", dim().apply_to("Restart gateway to apply: hermes gateway restart"));
        }
        "status" => {
            let config_path = get_hermes_home().join("gateway_config.yaml");
        ...
        _ => {
            println!("  {}", dim().apply_to("Usage: hermes whatsapp <setup|connect|status>"));
        }
    }
    println!();
