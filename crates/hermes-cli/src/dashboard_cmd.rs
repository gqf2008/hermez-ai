...
//! Dashboard TUI.
//!
//! Mirrors Python: hermes dashboard (interactive analytics dashboard)
...
pub fn cmd_dashboard_with_opts(_host: &str, _port: u16, _no_open: bool, _insecure: bool) -> anyhow::Result<()> {
    ...
    if db_path.exists() {
        if let Ok(db) = hermes_state::SessionDB::open(&db_path) {
    ...
    println!();
    println!("  {}", dim().apply_to("For detailed analytics, use: hermes insights"));
    println!("  {}", dim().apply_to("For session details, use: hermes sessions list"));
    println!();
