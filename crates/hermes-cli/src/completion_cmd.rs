...
/// Build a command tree matching the real `hermes` binary.
fn build_hermes_command() -> Command {
    let mut cmd = Command::new("hermes")
        .version(env!("CARGO_PKG_VERSION"))
        .about("Hermes Agent CLI")
    ...
    cmd = cmd
        ...
        .arg(clap::Arg::new("hermes_home").long("hermes-home").global(true).value_name("DIR"));
...
pub fn cmd_completion(shell: &str) -> anyhow::Result<()> {
    ...
    let mut cmd = build_hermes_command();
