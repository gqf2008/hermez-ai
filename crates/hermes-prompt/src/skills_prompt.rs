...
//! 2. Disk snapshot (~/.hermes/.skills_prompt_snapshot.json) validated by mtime/size
...
fn load_disabled_skills() -> HashSet<String> {
    ...
    if let Ok(config) = hermes_core::HermesConfig::load() {
...
fn parse_category_description(path: &Path) -> Option<(String, String)> {
    ...
    let skills_dir = hermes_core::get_hermes_home().join("skills");
