...
/// Context file priority names for .hermes.md search.
const HERMES_MD_NAMES: &[&str] = &[".hermes.md", "HERMES.md"];
...
fn load_context_file(path: &Path, label: &str, max_chars: usize) -> Option<String> {
    ...
    // Strip YAML frontmatter (e.g., from .hermes.md with model overrides)
...
pub fn build_context_files_prompt(cwd: &Path, skip_soul: bool) -> Option<String> {
    ...
    let project_context = find_hermes_md(cwd)
        .and_then(|p| load_context_file(&p, ".hermes.md", SOUL_CONTEXT_FILE_MAX_CHARS))
        .or_else(|| {
            // AGENTS.md
...
mod tests {
    ...
    fn test_build_system_prompt_basic() {
        ...
        assert!(result.system_prompt.contains("Hermes Agent"));
    ...
    fn test_e2e_prompt_with_user_system_prompt() {
        ...
        assert!(result.system_prompt.find("security") > result.system_prompt.find("Hermes Agent"));
    }
