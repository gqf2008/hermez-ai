...
async fn process_batch(
    ...
    for (prompt_index, entry) in entries {
        ...
        let agent_config = hermez_agent_engine::agent::AgentConfig {
        ...
        let mut agent = hermez_agent_engine::AIAgent::new(agent_config, Arc::clone(registry))
            .map_err(|e| {
                HermezError::new(
                    hermez_core::errors::ErrorCategory::InternalError,
        ...
        let trajectory = TrajectoryEntry {
            ...
            model: config.model.clone(),
            completed: turn_result.exit_reason == hermez_agent_engine::agent::ExitReason::Completed,
