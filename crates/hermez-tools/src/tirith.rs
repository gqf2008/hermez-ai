...
fn tirith_binary_path() -> PathBuf {
    get_hermez_home().join("bin").join("tirith")
}
...
pub fn check_command_security(command: &str) -> Result<SecurityCheckResult, String> {
    ...
    if !binary.exists() {
        return Err(format!(
            "Tirith not installed. Install with: `hermez tools install tirith` or run tirith from {}.",
            binary.display()
        ));
    }
...
mod tests {
    ...
    fn test_binary_path() {
        ...
        assert!(path.starts_with(get_hermez_home()));
    }
}
