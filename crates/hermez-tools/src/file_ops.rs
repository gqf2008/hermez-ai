...
/// HERMEZ_HOME guard — prevent reading internal Hermez cache files.
fn is_hermez_internal(resolved: &Path) -> bool {
    let hermez_home = match hermez_core::hermez_home::get_hermez_home().canonicalize() {
        Ok(p) => p,
        Err(_) => return false,
    };
    ...
    let rel = match resolved.strip_prefix(&hermez_home) {
...
fn read_file(path: &str, offset: usize, limit: usize) -> Value {
    ...
    // Hermez internal path guard
    match std::fs::canonicalize(file_path) {
        Ok(resolved) if is_hermez_internal(&resolved) => {
            return serde_json::json!({
                "error": format!("Access denied: {path} is an internal Hermez cache file and cannot be read directly to prevent prompt injection. Use the skills_list or skill_view tools instead.")
            });
        }
        _ => {}
    }
