...
fn block_on_browser<F>(f: F) -> Result<String, hermez_core::HermezError>
where
    F: std::future::Future<Output = Result<String, String>>,
{
    let handle = match tokio::runtime::Handle::try_current() {
        Ok(h) => h,
        Err(_) => {
            return Err(hermez_core::HermezError::new(
                hermez_core::ErrorCategory::ToolError,
...
pub fn handle_browser_tool(args: Value) -> Result<String, hermez_core::HermezError> {
    let action = args
        .get("action")
        .and_then(Value::as_str)
        .unwrap_or("navigate");
...
pub fn handle_browser(action: &str, args: &Value) -> Result<String, hermez_core::HermezError> {
    match action {
...
fn run_browser_json(
    ...
    if let Some(ref info) = session {
        ...
        match &*RESOLVED_BACKEND {
            ...
            _ => {
                cmd.arg("--session").arg(format!("hermez-{task_id}"));
            }
        }
    }
...
fn extract_relevant_content(snapshot_text: &str, user_task: Option<&str>) -> String {
    ...
    let response = hermez_llm::auxiliary_client::call_llm(
        Some("web_extract"),
        None,
        None,
        None,
...
fn browser_navigate(args: &Value) -> Result<String, hermez_core::HermezError> {
    ...
    if result.get("success").and_then(Value::as_bool).unwrap_or(false) {
        ...
        SESSION_MANAGER.register_local(&task_id, &format!("hermez-{task_id}"));
...
fn browser_navigate_camofox(url: &str, task_id: &str) -> Result<String, hermez_core::HermezError> {
...
fn browser_snapshot(args: &Value) -> Result<String, hermez_core::HermezError> {
    let task_id = get_task_id(args);
...
fn browser_click(args: &Value) -> Result<String, hermez_core::HermezError> {
...
fn browser_click_camofox(ref_id: &str, task_id: &str) -> Result<String, hermez_core::HermezError> {
    let ref_id = ref_id.to_string();
    let task_id = task_id.to_string();
    block_on_browser(camofox_click_async(ref_id, task_id))
}

fn browser_type(args: &Value) -> Result<String, hermez_core::HermezError> {
    let ref_id = match args.get("ref").and_then(Value::as_str) {
        Some(r) => r.to_string(),
...
fn browser_type_camofox(ref_id: &str, text: &str, task_id: &str) -> Result<String, hermez_core::HermezError> {
    let ref_id = ref_id.to_string();
...
fn browser_scroll(args: &Value) -> Result<String, hermez_core::HermezError> {
...
fn browser_scroll_camofox(direction: &str, task_id: &str) -> Result<String, hermez_core::HermezError> {
    let direction = direction.to_string();
    let task_id = task_id.to_string();
    block_on_browser(camofox_scroll_async(direction, task_id))
}

fn browser_back(_args: &Value) -> Result<String, hermez_core::HermezError> {
...
fn browser_back_camofox(task_id: &str) -> Result<String, hermez_core::HermezError> {
    let task_id = task_id.to_string();
    block_on_browser(camofox_back_async(task_id))
}

fn browser_press(args: &Value) -> Result<String, hermez_core::HermezError> {
...
fn browser_press_camofox(key: &str, task_id: &str) -> Result<String, hermez_core::HermezError> {
    let key = key.to_string();
    let task_id = task_id.to_string();
    block_on_browser(camofox_press_async(key, task_id))
}

fn browser_get_images(_args: &Value) -> Result<String, hermez_core::HermezError> {
    let task_id = get_task_id(_args);
    SESSION_MANAGER.update_activity(&task_id);
...
fn browser_vision(args: &Value) -> Result<String, hermez_core::HermezError> {
...
fn browser_vision_camofox(question: &str, _task_id: &str) -> Result<String, hermez_core::HermezError> {
...
fn browser_get_images_camofox(task_id: &str) -> Result<String, hermez_core::HermezError> {
    let task_id = task_id.to_string();
    block_on_browser(camofox_get_images_async(task_id))
}

fn browser_console(args: &Value) -> Result<String, hermez_core::HermezError> {
    ...
    if let Some(expr) = expression {
        ...
        match run_agent_browser(&["eval", "JSON.stringify({ errors: (window.__hermezConsole || []), logs: [] })"]) {
