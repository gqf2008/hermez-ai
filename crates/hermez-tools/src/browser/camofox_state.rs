//! Hermez-managed Camofox state helpers.
//!
//! Provides profile-scoped identity and state directory paths for Camofox
//! persistent browser profiles.  When managed persistence is enabled, Hermez
...
pub fn get_camofox_state_dir() -> PathBuf {
    hermez_core::get_hermez_home()
        .join(CAMOFOX_STATE_DIR_NAME)
        .join(CAMOFOX_STATE_SUBDIR)
}

/// Return the stable Hermez-managed Camofox identity for this profile.
///
/// The user identity is profile-scoped (same Hermez profile = same userId).
...
///
/// Profile-scoped: same Hermez profile always yields the same user_id,
...
mod tests {
    ...
    fn test_get_camofox_identity() {
        ...
        assert!(user_id.starts_with("hermez_"));
