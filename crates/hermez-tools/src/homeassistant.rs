...
pub fn handle_ha_list_entities(args: Value) -> Result<String, hermez_core::HermezError> {
...
pub fn handle_ha_get_state(args: Value) -> Result<String, hermez_core::HermezError> {
    let (url, token) = get_config();
    if token.is_empty() {
        return Ok(tool_error("HASS_TOKEN not set. Configure Home Assistant integration."));
    }
...
pub fn handle_ha_call_service(args: Value) -> Result<String, hermez_core::HermezError> {
    let (url, token) = get_config();
    if token.is_empty() {
        return Ok(tool_error("HASS_TOKEN not set. Configure Home Assistant integration."));
    }
...
pub fn handle_ha_list_services(args: Value) -> Result<String, hermez_core::HermezError> {
    let (url, token) = get_config();
    if token.is_empty() {
        return Ok(tool_error("HASS_TOKEN not set. Configure Home Assistant integration."));
    }
