...
const CONTAINER_BASE: &str = "/root/.hermez";
...
///
/// `relative_path` is relative to HERMEZ_HOME (e.g. "google_token.json").
...
/// The resolved host path must remain inside HERMEZ_HOME so that a malicious
...
pub fn register_credential_file(relative_path: &str, container_base: &str) -> bool {
    ...
    if Path::new(relative_path).is_absolute() {
        tracing::warn!(
            "credential_files: rejected absolute path {:?} (must be relative to HERMEZ_HOME)",
            relative_path
        );
        return false;
    }
    ...
    let hermez_home_resolved = match hermez_home.canonicalize() {
        Ok(p) => p,
        Err(_) => {
            tracing::warn!("credential_files: HERMEZ_HOME does not exist: {}", hermez_home.display());
            return false;
        }
    };

    if !resolved.starts_with(&hermez_home_resolved) {
        tracing::warn!(
            "credential_files: rejected path traversal {:?} (resolves to {}, outside HERMEZ_HOME {})",
            relative_path,
            resolved.display(),
            hermez_home_resolved.display()
        );
        return false;
    }
...
fn load_config_files() -> Vec<MountEntry> {
    CONFIG_FILES
        .get_or_init(|| {
            ...
            let hermez_home_resolved = hermez_home
                .canonicalize()
                .unwrap_or_else(|_| hermez_home.clone());
                ...
                .and_then(|c| c.as_sequence())
            {
                for item in cred_files {
                    if let Some(rel) = item.as_str() {
                        ...
                        let host_path = hermez_home.join(rel);
                        let resolved = match host_path.canonicalize() {
                            Ok(p) => p,
                            Err(_) => continue,
                        };
...
pub fn get_cache_directory_mounts(container_base: &str) -> Vec<MountEntry> {
    ...
    for (new_subpath, _old_name) in CACHE_DIRS {
        let host_dir = get_hermez_home().join(new_subpath);
        if host_dir.is_dir() {
...
mod tests {
    ...
    fn test_cache_directory_mounts_empty() {
        ...
        assert!(mounts.iter().all(|m| m.container_path.starts_with("/root/.hermez/cache")));
    }
}
