...
pub fn check_session_requirements() -> bool {
    let hermez_home = hermez_core::hermez_home::get_hermez_home();
    hermez_home.join("sessions.db").exists()
}
...
fn open_session_db() -> Result<hermez_state::SessionDB, String> {
    let hermez_home = hermez_core::hermez_home::get_hermez_home();
    ...
    hermez_state::SessionDB::open(&db_path)
...
async fn summarize_session(
    ...
    for attempt in 0..max_retries {
        match hermez_llm::client::call_llm(request.clone()).await {
            Ok(resp) => {
...
pub fn handle_session_search(args: Value) -> Result<String, hermez_core::HermezError> {
    let query = args
        .get("query")
