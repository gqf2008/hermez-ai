...
//! Jobs are stored in JSON at `~/.hermez/cron/jobs.json`.
...
/// On-disk format for the jobs file (compatible with `hermez-cron`).
#[derive(Debug, Clone, Serialize, Deserialize)]
struct JobsFile {
    jobs: Vec<CronJob>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
fn jobs_file() -> PathBuf {
    get_hermez_home().join("cron").join("jobs.json")
}

/// Load all cron jobs.
///
/// Supports both the new object format (`{"jobs":[...]}`) used by
/// `hermez-cron` and the legacy array format for backward compatibility.
...
/// so that it stays compatible with `hermez-cron`.
...
pub fn handle_cronjob(args: Value) -> Result<String, hermez_core::HermezError> {
    let action = args
        .get("action")
...
fn handle_create(args: &Value) -> Result<String, hermez_core::HermezError> {
    ...
    if let Some(ref script_path) = script {
        ...
        let scripts_dir = hermez_home.join("scripts");
...
fn handle_list(args: &Value) -> Result<String, hermez_core::HermezError> {
...
fn handle_update(args: &Value) -> Result<String, hermez_core::HermezError> {
...
fn handle_pause(args: &Value) -> Result<String, hermez_core::HermezError> {
    let job_id = match args.get("job_id").and_then(Value::as_str) {
        Some(id) => id.to_string(),
...
fn handle_resume(args: &Value) -> Result<String, hermez_core::HermezError> {
    let job_id = match args.get("job_id").and_then(Value::as_str) {
        Some(id) => id.to_string(),
...
fn handle_remove(args: &Value) -> Result<String, hermez_core::HermezError> {
    let job_id = match args.get("job_id").and_then(Value::as_str) {
        Some(id) => id.to_string(),
...
fn handle_run(args: &Value) -> Result<String, hermez_core::HermezError> {
    let job_id = match args.get("job_id").and_then(Value::as_str) {
        Some(id) => id.to_string(),
...
pub fn check_cron_enabled() -> bool {
    if let Ok(config) = HermezConfig::load() {
        config.cron.enabled
    } else {
        false
    }
        ...
        serde_json::json!({
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "script": { "type": "string", "description": "Path to a script to run before the job (must be in ~/.hermez/scripts/)." },
...
mod tests {
    ...
    fn test_load_jobs_empty() {
        // With default HERMEZ_HOME, jobs file may or may not exist.
