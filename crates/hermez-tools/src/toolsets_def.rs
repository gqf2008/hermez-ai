...
/// The master list of Hermez core tools shared across all messaging platforms.
///
/// Mirrors the Python `_HERMEZ_CORE_TOOLS` list.
pub const HERMEZ_CORE_TOOLS: &[&str] = &[
    // Web
    "web_search",
    "web_extract",
...
pub fn toolsets() -> HashMap<&'static str, ToolsetDef> {
    ...
    // Platform toolsets — all reference HERMEZ_CORE_TOOLS.
    // Mirrors Python toolsets.py:68-396.

    map.insert("hermez-acp", ToolsetDef {
    ...
    map.insert("hermez-api-server", ToolsetDef {
    ...
    map.insert("hermez-cli", ToolsetDef {
        description: "CLI platform — all core tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-telegram", ToolsetDef {
        description: "Telegram platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-discord", ToolsetDef {
        description: "Discord platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-whatsapp", ToolsetDef {
        description: "WhatsApp platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-slack", ToolsetDef {
        description: "Slack platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-signal", ToolsetDef {
        description: "Signal platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-bluebubbles", ToolsetDef {
        description: "BlueBubbles (iMessage) platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-homeassistant", ToolsetDef {
        description: "Home Assistant platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-email", ToolsetDef {
        description: "Email platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-sms", ToolsetDef {
        description: "SMS platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-mattermost", ToolsetDef {
        description: "Mattermost platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-matrix", ToolsetDef {
        description: "Matrix platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-dingtalk", ToolsetDef {
        description: "DingTalk platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-feishu", ToolsetDef {
        description: "Feishu platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-weixin", ToolsetDef {
        description: "Weixin (WeChat) platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-qqbot", ToolsetDef {
        description: "QQ Bot platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-wecom", ToolsetDef {
        description: "WeCom platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-wecom-callback", ToolsetDef {
        description: "WeCom callback platform tools",
        tools: HERMEZ_CORE_TOOLS,
        includes: &[],
    });

    map.insert("hermez-webhook", ToolsetDef {
    ...
    map.insert("hermez-gateway", ToolsetDef {
        description: "Gateway — union of all platform toolsets",
        tools: &[],
        includes: &[
            "hermez-telegram", "hermez-discord", "hermez-whatsapp",
            "hermez-slack", "hermez-signal", "hermez-bluebubbles",
            "hermez-homeassistant", "hermez-email", "hermez-sms",
            "hermez-mattermost", "hermez-matrix", "hermez-dingtalk",
            "hermez-feishu", "hermez-weixin", "hermez-qqbot",
            "hermez-wecom", "hermez-wecom-callback", "hermez-webhook",
        ],
    });
...
pub fn resolve_toolset(name: &str) -> Option<Vec<String>> {
    ...
    let actual_name = match name {
        "all" | "*" => return Some(HERMEZ_CORE_TOOLS.iter().map(|s| s.to_string()).collect()),
        _ => name,
    };
...
mod tests {
    ...
    fn test_platform_toolsets_exist() {
        let platforms = [
            "hermez-cli", "hermez-acp", "hermez-api-server",
            "hermez-telegram", "hermez-discord",
            "hermez-whatsapp", "hermez-slack", "hermez-signal",
            "hermez-bluebubbles", "hermez-homeassistant", "hermez-email",
            "hermez-sms", "hermez-mattermost", "hermez-matrix",
            "hermez-dingtalk", "hermez-feishu", "hermez-weixin",
            "hermez-qqbot", "hermez-wecom", "hermez-wecom-callback",
            "hermez-webhook", "hermez-gateway",
        ];
        let ts = toolsets();
    ...
    fn test_platform_toolset_has_core_tools() {
        let tools = resolve_toolset("hermez-telegram").unwrap();
        assert!(tools.contains(&"web_search".to_string()));
    ...
    fn test_gateway_includes_all_platforms() {
        let tools = resolve_toolset("hermez-gateway").unwrap();
