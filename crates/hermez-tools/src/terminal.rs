...
//! Environment selection via `HERMEZ_TERMINAL_BACKEND` env var or config.
//!
//! Features:
...
fn transform_sudo_command(command: &str) -> (String, Option<String>) {
    ...
    let sudo_password = if has_configured_password {
        ...
        cached.clone()
    } else if std::env::var("HERMEZ_INTERACTIVE").is_ok() {
...
fn prompt_for_sudo_password(timeout_seconds: u64) -> String {
    ...
    if std::env::var("HERMEZ_INTERACTIVE").is_err() {
        return String::new();
    }
...
fn handle_sudo_failure(output: &str, _env_type: &str) -> String {
    let is_gateway = std::env::var("HERMEZ_GATEWAY_SESSION").is_ok();
    if !is_gateway {
        return output.to_string();
    }
...
fn check_all_guards(command: &str, env_type: &str) -> GuardResult {
    let gateway_ask = std::env::var("HERMEZ_GATEWAY_ASK_MODE").is_ok();
    ...
    // 2. Approval mode
    let mode_str = std::env::var("HERMEZ_APPROVAL_MODE")
    ...
    let session_id = std::env::var("HERMEZ_SESSION_ID").ok();
...
fn resolve_env_config(override_type: Option<&str>) -> EnvConfig {
    // Check env var first (TERMINAL_ENV or HERMEZ_TERMINAL_BACKEND)
    let env_type = override_type
        .map(|s| s.to_string())
        .or_else(|| std::env::var("HERMEZ_TERMINAL_BACKEND").ok())
    ...
    let terminal_cfg = HermezConfig::load()
        .ok()
        .map(|c| c.terminal);
...
pub fn handle_terminal(args: Value) -> Result<String, hermez_core::HermezError> {
...
mod tests {
    ...
    fn test_check_all_guards_gateway_ask_mode() {
        std::env::set_var("HERMEZ_GATEWAY_ASK_MODE", "1");
        let result = check_all_guards("rm -rf /", "local");
        std::env::remove_var("HERMEZ_GATEWAY_ASK_MODE");
        assert!(!result.approved);
