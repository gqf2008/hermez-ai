...
pub async fn web_extract_url(url: &str) -> Result<String, String> {
    ...
    let resp = client
        .get(url)
        .header("User-Agent", "Mozilla/5.0 (compatible; HermezAgent/1.0)")
        .send()
        .await
...
Your goal is to preserve ALL important information while reducing length. Never lose key facts, figures, insights, or actionable information. Make it scannable and well-organized.";
    ...
    let effective_model = model
        ...
        .or_else(|| hermez_llm::auxiliary_client::get_default_aux_model("openrouter"))
        .unwrap_or_else(|| "google/gemini-3-flash-preview".to_string());

    let request = hermez_llm::auxiliary_client::AuxiliaryRequest {
    ...
    match hermez_llm::auxiliary_client::call_auxiliary(request).await {
        Ok(response) => {
...
async fn process_large_content_chunked(
    ...
    let mut final_summary = match hermez_llm::auxiliary_client::call_auxiliary(request).await {
        Ok(response) => {
            let text = response.content;
            if text.is_empty() {
...
pub fn handle_web_search(args: Value) -> Result<String, hermez_core::HermezError> {
    let query = args
        .get("query")
        .and_then(Value::as_str)
        .ok_or_else(|| {
...
pub fn handle_web_extract(args: Value) -> Result<String, hermez_core::HermezError> {
    let urls = args
        .get("url")
        .and_then(|v| {
...
pub fn handle_web_crawl(args: Value) -> Result<String, hermez_core::HermezError> {
    let url = args
        .get("url")
        .and_then(Value::as_str)
        .ok_or_else(|| {
