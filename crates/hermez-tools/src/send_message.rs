...
pub fn check_send_message_requirements() -> bool {
    ...
    std::env::var("HERMEZ_GATEWAY_CONFIG").is_ok()
...
pub fn handle_send_message(args: Value) -> Result<String, hermez_core::HermezError> {
    let action = args
        .get("action")
...
fn handle_send(args: &Value) -> Result<String, hermez_core::HermezError> {
