...
//! `~/.hermez/skills/` and uses a manifest to track which skills
...
/// Skills directory: ~/.hermez/skills/
fn skills_dir() -> &'static Path {
    static DIR: Lazy<PathBuf> = Lazy::new(|| hermez_core::get_hermez_home().join("skills"));
    DIR.as_path()
}

/// Manifest file: ~/.hermez/skills/.bundled_manifest
...
///
/// Checks `HERMEZ_BUNDLED_SKILLS` env var first (set by Nix wrapper),
/// then falls back to the repo-relative `skills/` path.
pub fn get_bundled_dir() -> PathBuf {
    if let Ok(env) = std::env::var("HERMEZ_BUNDLED_SKILLS") {
        if !env.is_empty() {
            return PathBuf::from(env);
        }
    }
    // Fallback: relative to this crate's location in the workspace
    // hermez-rs/crates/hermez-tools/ -> hermez-rs/skills/
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    // Go up 3 levels: hermez-tools -> crates -> hermez-rs
    manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
...
/// Sync bundled skills into ~/.hermez/skills/.
///
...
mod tests {
    ...
    fn test_sync_missing_bundled_dir() {
        ...
        std::env::set_var("HERMEZ_BUNDLED_SKILLS", "/nonexistent/path/xyz");
        let result = sync_skills(true);
        assert_eq!(result.total_bundled, 0);
        assert!(result.copied.is_empty());
        std::env::remove_var("HERMEZ_BUNDLED_SKILLS");
    }
}
