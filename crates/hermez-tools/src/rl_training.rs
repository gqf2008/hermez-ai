...
fn find_atropos_root() -> Option<PathBuf> {
    // Check relative to hermez-agent root
    // manifest_dir = hermez-rs/crates/hermez-tools
    // parent1 = hermez-rs/crates
    // parent2 = hermez-rs
    // parent3 = hermez-agent (root)
...
fn handle_rl_action(action: &str, args: &Value) -> Result<String, hermez_core::HermezError> {
    match action {
...
fn rl_list_environments() -> Result<String, hermez_core::HermezError> {
...
fn rl_select_environment(args: &Value) -> Result<String, hermez_core::HermezError> {
...
fn rl_get_current_config() -> Result<String, hermez_core::HermezError> {
...
fn rl_edit_config(args: &Value) -> Result<String, hermez_core::HermezError> {
...
fn rl_start_training(_args: &Value) -> Result<String, hermez_core::HermezError> {
    let mut state = RL_STATE.lock().unwrap();
...
fn spawn_training_process(run_id: &str, env_name: &str) -> (String, Option<Child>, Option<PathBuf>) {
    ...
    // Create log file
    let log_dir = std::env::temp_dir().join("hermez_rl_logs");
...
fn rl_check_status(args: &Value) -> Result<String, hermez_core::HermezError> {
...
fn rl_stop_training(args: &Value) -> Result<String, hermez_core::HermezError> {
    let run_id = match args.get("run_id").and_then(Value::as_str) {
        Some(r) => r.to_string(),
...
fn rl_get_results(args: &Value) -> Result<String, hermez_core::HermezError> {
    let run_id = match args.get("run_id").and_then(Value::as_str) {
        Some(r) => r.to_string(),
...
fn rl_list_runs() -> Result<String, hermez_core::HermezError> {
    let state = RL_STATE.lock().unwrap();
...
fn rl_test_inference(args: &Value) -> Result<String, hermez_core::HermezError> {
...
/// Handle RL tool call.
pub fn handle_rl(args: Value) -> Result<String, hermez_core::HermezError> {
    let action = args
        .get("action")
