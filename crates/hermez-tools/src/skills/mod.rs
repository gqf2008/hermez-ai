...
/// Get the skills directory: ~/.hermez/skills/
fn skills_dir() -> PathBuf {
    hermez_core::get_hermez_home().join("skills")
}
...
fn all_skills_dirs() -> Vec<PathBuf> {
    ...
    // 1. Builtin skills from HERMEZ_BUILTIN_SKILLS env var
    if let Ok(builtin) = std::env::var("HERMEZ_BUILTIN_SKILLS") {
...
fn get_disabled_skill_names() -> std::collections::BTreeSet<String> {
    ...
    let Ok(config) = hermez_core::config::HermezConfig::load() else {
        return disabled;
    };
    if let Ok(platform) = std::env::var("HERMEZ_PLATFORM") {
...
pub fn handle_skills_list(args: Value) -> Result<String, hermez_core::HermezError> {
    ...
    if !dir.exists() {
        ...
        return Ok(serde_json::json!({
            ...
            "skills": [],
            "categories": [],
            "message": "No skills found. Skills directory created at ~/.hermez/skills/",
        })
        .to_string());
    }
...
pub fn handle_skill_view(args: Value) -> Result<String, hermez_core::HermezError> {
    let name = args
        .get("name")
    ...
    if disabled.contains(&skill_name) {
        return Ok(serde_json::json!({
            "success": false,
            "error": format!("Skill '{skill_name}' is disabled. Enable it with `hermez skills` or inspect the files directly on disk."),
        })
        .to_string());
    }
...
fn extract_tags_and_related(fm: &SkillFrontmatter) -> (Vec<String>, Vec<String>) {
    let hermez_meta = fm
        .metadata
        .as_ref()
        .and_then(|m| m.get("hermez"))
        .and_then(|h| h.as_object());

    let tags_raw = hermez_meta
        .and_then(|h| h.get("tags"))
        .or_else(|| fm.metadata.as_ref().and_then(|m| m.get("tags")));

    let related_raw = hermez_meta
...
pub fn handle_skills_categories(args: Value) -> Result<String, hermez_core::HermezError> {
...
pub fn handle_skill_manage(args: Value) -> Result<String, hermez_core::HermezError> {
    let action = args
        .get("action")
        .and_then(Value::as_str)
        .unwrap_or("");
    ...
    let skills_root = hermez_home.join("skills");
...
fn handle_skill_create(skills_root: &Path, name: &str, args: &Value) -> Result<String, hermez_core::HermezError> {
...
fn handle_skill_edit(skills_root: &Path, name: &str, args: &Value) -> Result<String, hermez_core::HermezError> {
    let content = args.get("content").and_then(Value::as_str).unwrap_or("");
    if content.is_empty() {
...
fn handle_skill_patch(skills_root: &Path, name: &str, args: &Value) -> Result<String, hermez_core::HermezError> {
...
fn handle_skill_delete(skills_root: &Path, name: &str) -> Result<String, hermez_core::HermezError> {
    let Some(skill_dir) = find_skill_dir(skills_root, name)
        .or_else(|| find_skill_anywhere(skills_root, name))
    else {
        return Ok(tool_error(format!("Skill not found: {name}")));
...
fn handle_skill_write_file(skills_root: &Path, name: &str, args: &Value) -> Result<String, hermez_core::HermezError> {
...
fn handle_skill_remove_file(skills_root: &Path, name: &str, args: &Value) -> Result<String, hermez_core::HermezError> {
    let file_path = args.get("file_path").and_then(Value::as_str);
...
pub fn extract_skill_config_vars(fm: &Value) -> Vec<SkillConfigVar> {
    ...
    let hermez = metadata
        .and_then(|m| m.get("hermez"))
        .and_then(|h| h.as_object());
    let raw = match hermez.and_then(|h| h.get("config")) {
        Some(v) => v,
        None => return Vec::new(),
    };
...
pub fn resolve_skill_config_values(vars: &[SkillConfigVar]) -> BTreeMap<String, String> {
    let config_path = hermez_core::get_hermez_home().join("config.yaml");
...
/// Scan ~/.hermez/skills/ and return a mapping of "/command" -> skill info.
///
    ...
    if let Some(fm_val) = parse_frontmatter_value(&content) {
        ...
        if !config_vars.is_empty() {
            ...
            if !resolved.is_empty() {
                ...
                parts.push("[Skill config (from ~/.hermez/config.yaml):".to_string());
        ...
        if let Some(fm_val) = parse_frontmatter_value(&content) {
            ...
            if !config_vars.is_empty() {
                ...
                if !resolved.is_empty() {
                    ...
                    skill_parts.push("[Skill config (from ~/.hermez/config.yaml):".to_string());
                    for (key, value) in &resolved {
                        let display_val = if value.is_empty() {
                            "(not set)".to_string()
                        } else {
...
pub fn register_skills_tools(registry: &mut crate::registry::ToolRegistry) {
    ...
    registry.register(
        ...
        serde_json::json!({
            "name": "skill_manage",
            "description": "Manage skills (create, update, delete). Skills are your procedural memory — reusable approaches for recurring task types. New skills go to ~/.hermez/skills/; existing skills can be modified wherever they live. Actions: create (full SKILL.md + optional category), edit (full rewrite), patch (find/replace in SKILL.md or linked files), delete (remove entire skill), write_file (create/update linked file within a skill), remove_file (delete linked file from skill).",
            "parameters": {
                "type": "object",
                "properties": {
...
mod tests {
    ...
    // Shared test base dir — set once before any test accesses get_hermez_home()
    ...
    fn init_hermez_home() -> bool {
        // Try to set hermez home to our shared test base dir.
        ...
        hermez_core::hermez_home::set_hermez_home(test_base()).is_ok()
    }
    ...
    fn test_parse_frontmatter_value() {
        ...
        assert!(val["metadata"]["hermez"]["config"].is_array());
    }
...
"#,
    ...
    fn test_build_plan_path() {
        ...
        assert!(path_str.contains(".hermez"));
    ...
    fn test_scan_skill_commands_empty() {
        ...
        let commands = scan_skill_commands();
        // May or may not be empty depending on real ~/.hermez/skills
