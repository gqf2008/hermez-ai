...
///
/// Activated via environment variable (e.g., `HERMEZ_DEBUG_TOOLS=true`).
...
impl DebugSession {
    ...
    pub fn new(tool_name: &str, env_var: &str) -> Self {
        ...
        let log_dir = get_hermez_home().join("logs");
...
mod tests {
    ...
    fn test_disabled_session() {
        let mut session = DebugSession::new("test_tool", "HERMEZ_DEBUG_TEST_TOOL");
    ...
    fn test_enabled_session() {
        std::env::set_var("HERMEZ_DEBUG_TEST_TOOL_ENABLED", "true");
        let mut session = DebugSession::new("test_tool", "HERMEZ_DEBUG_TEST_TOOL_ENABLED");
        assert!(session.enabled);
        ...
        std::env::remove_var("HERMEZ_DEBUG_TEST_TOOL_ENABLED");
    }
}
