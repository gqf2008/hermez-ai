...
pub const STORAGE_DIR: &str = "/tmp/hermez-results";
...
static STORAGE_PATH: LazyLock<PathBuf> = LazyLock::new(|| {
    std::env::var("HERMEZ_TOOL_RESULTS_DIR")
        .map(PathBuf::from)
...
mod tests {
    ...
    fn test_get_storage_dir() {
        ...
        assert!(dir.to_string_lossy().contains("hermez-results")
            || std::env::var("HERMEZ_TOOL_RESULTS_DIR").is_ok());
    }
