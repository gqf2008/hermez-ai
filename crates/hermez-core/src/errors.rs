//! Unified error types for the Hermez Agent system.
//!
...
/// Result alias for HermezError
pub type Result<T> = std::result::Result<T, HermezError>;
...
/// The unified error type for all Hermez operations.
#[derive(Error, Debug)]
pub struct HermezError {
...
impl fmt::Display for HermezError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
impl From<anyhow::Error> for HermezError {
...
impl From<std::io::Error> for HermezError {
...
impl From<serde_json::Error> for HermezError {
...
impl From<String> for HermezError {
    fn from(err: String) -> Self {
        Self::new(ErrorCategory::InternalError, err)
    }
}

impl From<&str> for HermezError {
...
mod tests {
    ...
    #[test]
    fn test_error_new() {
        let err = HermezError::new(ErrorCategory::ConfigError, "bad config");
    ...
    fn test_error_with_source() {
        ...
        let err = HermezError::with_source(ErrorCategory::FileError, "read failed", inner.into());
    ...
    fn test_error_api_with_details() {
        ...
        let err = HermezError::api(ErrorCategory::ApiError, "rate limited", details);
    ...
    fn test_error_retryable_fallback_without_api_details() {
        let network_err = HermezError::new(ErrorCategory::NetworkError, "timeout");
        assert!(network_err.is_retryable());
        assert!(!network_err.should_rotate());
        assert!(!network_err.should_fallback());

        let api_err = HermezError::new(ErrorCategory::ApiError, "bad request");
        assert!(api_err.is_retryable());
        assert!(api_err.should_fallback());

        let config_err = HermezError::new(ErrorCategory::ConfigError, "missing");
    ...
    fn test_error_display() {
        let err = HermezError::new(ErrorCategory::ToolError, "hammer missed");
        let s = format!("{err}");
    ...
    fn test_from_anyhow() {
        ...
        let err: HermezError = anyhow_err.into();
    ...
    fn test_from_io_error() {
        ...
        let err: HermezError = io_err.into();
        assert_eq!(err.category, ErrorCategory::InternalError);
    ...
    #[test]
    fn test_from_string() {
        let err: HermezError = "plain string error".to_string().into();
        assert_eq!(err.category, ErrorCategory::InternalError);
    ...
    #[test]
    fn test_from_str() {
        let err: HermezError = "slice error".into();
        assert_eq!(err.category, ErrorCategory::InternalError);
