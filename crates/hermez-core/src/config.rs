//! Hermez configuration types.
//!
//! Mirrors the Python `hermez_cli/config.py` DEFAULT_CONFIG and OPTIONAL_ENV_VARS.
...
use crate::errors::{ErrorCategory, HermezError, Result};
use crate::hermez_home::get_hermez_home;

/// Current config schema version.
/// Kept at 18 to stay aligned with Python `hermez_cli/config.py`.
...
///
/// Mirrors the `~/.hermez/config.yaml` schema. Fields are optional to support
/// partial configs where missing values fall back to defaults or env vars.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct HermezConfig {
...
pub struct TerminalConfig {
    ...
    /// Sudo password for sudo -S (also from HERMEZ_SUDO_PASSWORD env)
...
///
/// Mirrors Python `_expand_env_vars` in `hermez_cli/config.py:2487`.
...
///
/// Mirrors Python `migrate_config` in `hermez_cli/config.py:2037`.
...
impl HermezConfig {
    ...
    ///
    /// Reads from `~/.hermez/config.yaml` or `./cli-config.yaml` (local override).
    ...
    pub fn load() -> Result<Self> {
        ...
        let content = std::fs::read_to_string(path)
            .map_err(|e| HermezError::with_source(
        ...
        let config: HermezConfig = serde_json::from_value(json)
            .map_err(|e| HermezError::with_source(
                ErrorCategory::ConfigError,
...
mod tests {
    ...
    fn test_default_config() {
        let config = HermezConfig::default();
    ...
    fn test_config_roundtrip() {
        ...
        let loaded: HermezConfig = serde_yaml::from_str(&yaml).unwrap();
