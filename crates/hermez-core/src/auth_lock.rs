...
//!
//! Prevents concurrent writes from multiple Hermez processes (or threads)
...
/// Return the path to the auth lock file (`~/.hermez/auth.lock`).
fn auth_lock_path() -> PathBuf {
    get_hermez_home().join("auth.lock")
}
