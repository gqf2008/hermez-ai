...
pub struct ModalEnvironment {
    ...
    /// TODO: Use `hermes_constants::get_hermes_home() / "modal_snapshots.json"`.
    #[allow(dead_code)]
...
impl ModalEnvironment {
    ...
    ///    `{HERMES_HOME}/modal_snapshots.json` under key `"direct:{task_id}"`.
    ...
    fn _load_snapshot(&self, _task_id: &str) -> Option<(String, bool)> {
        // TODO: Read from `{HERMES_HOME}/modal_snapshots.json`.
    ...
    fn _save_snapshot(&self, _task_id: &str, _snapshot_id: &str) {
        // TODO: Write to `{HERMES_HOME}/modal_snapshots.json`.
