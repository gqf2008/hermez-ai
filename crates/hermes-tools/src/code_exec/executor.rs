...
use hermes_core::errors::ErrorCategory;
...
fn execute_with_uds(
    ...
    // Generate hermes_tools.py module
    let module_code = sandbox::generate_hermes_tools_module(tool_names, "uds");
    ...
    // Write hermes_tools.py and script.py
    std::fs::write(sandbox_dir.join("hermes_tools.py"), &module_code).map_err(|e| {
        hermes_core::HermesError::new(
            ErrorCategory::ToolError,
            format!("Failed to write hermes_tools.py: {e}"),
        )
    })?;
    ...
    child_env.push(("HERMES_RPC_SOCKET".to_string(), socket_path.clone()));
    ...
    let output = Command::new(python)
        ...
        .output()
        .map_err(|e| {
            hermes_core::HermesError::new(ErrorCategory::ToolError, format!("Failed to run Python: {e}"))
        })?;
...
fn execute_with_file_rpc(
    ...
    // Generate hermes_tools.py module
    let module_code = sandbox::generate_hermes_tools_module(tool_names, "file");
    ...
    // Build sanitized environment
    let mut child_env: Vec<(String, String)> = sandbox::sanitize_env().into_iter().collect();
    child_env.push(("HERMES_RPC_DIR".to_string(), rpc_dir));
    child_env.push(("PYTHONDONTWRITEBYTECODE".to_string(), "1".to_string()));
...
fn process_output(output: &std::process::Output) -> Result<String, hermes_core::HermesError> {
