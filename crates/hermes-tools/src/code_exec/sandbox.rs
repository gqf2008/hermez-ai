//! Sandbox module generation — generates `hermes_tools.py` stub module.
//!
...
/// Generate the `hermes_tools.py` module source.
pub fn generate_hermes_tools_module(enabled_tools: &[String], transport: &str) -> String {
    let mut module = String::new();

    // Module header
    module.push_str("# Auto-generated hermes_tools module for code execution sandbox.\n");
...
def _connect():
    ...
    if not sock_path:
        raise RuntimeError("HERMES_RPC_SOCKET not set")
...
def _call(tool_name, args):
    ...
    global _seq
    rpc_dir = os.environ.get("HERMES_RPC_DIR")
    if not rpc_dir:
        raise RuntimeError("HERMES_RPC_DIR not set")
...
mod tests {
    ...
    fn test_generate_module_file() {
        ...
        let module = generate_hermes_tools_module(&tools, "file");

        assert!(module.contains("HERMES_RPC_DIR"));
    ...
    fn test_generate_module_empty_tools() {
        let module = generate_hermes_tools_module(&[], "uds");
