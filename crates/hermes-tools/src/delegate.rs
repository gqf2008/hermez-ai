...
///
/// Implemented by `hermes-agent-engine` (or the main binary) to avoid a
/// circular dependency between `hermes-tools` and `hermes-agent-engine`.
...
///
/// Called once at startup (e.g., from `hermes-agent-engine` or the main binary).
