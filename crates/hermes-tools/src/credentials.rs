...
///
/// The `relative_path` is relative to the Hermes home directory.
...
pub fn register_credential_file(relative_path: &str, container_base: &str) -> Result<(), String> {
    ...
    // Resolve and verify it stays within HERMES_HOME
    ...
    if !resolved.starts_with(&hermes_home) {
        return Err(format!(
            "Credential path resolves outside HERMES_HOME: {}",
            resolved.display()
        ));
    }
...
pub fn register_skill_credentials(skill_dir: &Path, frontmatter_creds: Option<&serde_json::Value>) {
    let container_base = "/root/.hermes";
    ...
    if let Some(Value::Array(entries)) = frontmatter_creds {
        for entry in entries {
            ...
            if let Some(rel_path) = entry.as_str() {
                ...
                if cred_path.exists() {
                    // Resolve to a path relative to HERMES_HOME
                    let hermes_home = get_hermes_home();
                    if let Ok(relative) = cred_path.strip_prefix(&hermes_home) {
...
pub fn get_skills_directory_mount() -> Option<HashMap<String, String>> {
    ...
    if skills_dir.exists() {
        ...
        m.insert("container_path".to_string(), "/root/.hermes/skills".to_string());
        Some(m)
    } else {
        None
    }
...
pub fn get_cache_directory_mounts() -> Vec<HashMap<String, String>> {
    let hermes_home = get_hermes_home();
    let cache_dirs = [
        ("documents", "/root/.hermes/documents"),
        ("images", "/root/.hermes/images"),
        ("audio", "/root/.hermes/audio"),
        ("screenshots", "/root/.hermes/screenshots"),
    ];

    cache_dirs
        .iter()
        .filter_map(|(dir, container_path)| {
            let host_path = hermes_home.join(dir);
            if host_path.exists() {
                let mut m = HashMap::new();
                m.insert(
                    "host_path".to_string(),
...
mod tests {
    ...
    fn test_register_absolute_path_rejected() {
        let result = register_credential_file("/etc/passwd", "/root/.hermes");
    ...
    fn test_register_traversal_rejected() {
        let result = register_credential_file("../../.ssh/id_rsa", "/root/.hermes");
        assert!(result.is_err());
    ...
    fn test_register_nonexistent_file() {
        let result = register_credential_file("nonexistent/file.txt", "/root/.hermes");
        assert!(result.is_err());
    ...
    fn test_register_valid_credential_file() {
        // Create a temp file inside HERMES_HOME
        let tmp = std::env::temp_dir();
        let hermes_home = tmp.join("test_cred_home2");
        let _ = std::fs::create_dir_all(&hermes_home);

        let cred_file = hermes_home.join("test_cred.txt");
        std::fs::write(&cred_file, "secret").unwrap();

        // Try to set HERMES_HOME — may fail if already cached
        let set_ok = hermes_core::hermes_home::set_hermes_home(&hermes_home).is_ok();
        if !set_ok {
            // Already cached, skip this test
            return;
        }

        let result = register_credential_file("test_cred.txt", "/root/.hermes");
        assert!(result.is_ok());
    ...
    fn test_register_from_json_entries() {
        ...
        register_credential_files(&entries, "/root/.hermes");
    ...
    fn test_get_cache_directory_mounts_nonexistent() {
        // With default HERMES_HOME, cache dirs likely don't exist
    ...
    fn test_get_skills_directory_mount_nonexistent() {
        // With default HERMES_HOME, skills dir may or may not exist
