...
fn shadow_repo_path(working_dir: &str) -> PathBuf {
    ...
    get_hermes_home().join("checkpoints").join(&hash[..16])
}
...
async fn init_shadow_repo(shadow_repo: &Path, working_dir: &str) -> Result<(), String> {
    ...
    let _ = run_git(&["config", "user.email", "hermes@local"], shadow_repo, working_dir).await;
    let _ = run_git(&["config", "user.name", "Hermes Checkpoint"], shadow_repo, working_dir).await;
    ...
    let _ = std::fs::write(
        shadow_repo.join("HERMES_WORKDIR"),
