...
//! - **local_command** — user-configured CLI (via `HERMES_LOCAL_STT_COMMAND`)
//!
...
const LOCAL_STT_COMMAND_ENV: &str = "HERMES_LOCAL_STT_COMMAND";
const LOCAL_STT_LANGUAGE_ENV: &str = "HERMES_LOCAL_STT_LANGUAGE";
...
async fn call_openai_compat_api(
    ...
    let boundary = "----HermesSTIBoundary";
        ...
        return TranscriptionResult {
            ...
            error: Some(
                "No STT provider available. Set OPENAI_API_KEY, GROQ_API_KEY, \
                 MISTRAL_API_KEY, or HERMES_LOCAL_STT_COMMAND."
                    .to_string(),
            ),
        };
    }
