...
fn get_jobs_path() -> PathBuf {
    ...
    if let Ok(base) = std::env::var("HERMES_CRON_TEST_DIR") {
...
mod tests {
    ...
    fn test_job_store_crud() {
        ...
        let test_dir = std::env::temp_dir().join("hermes_cron_test_crud");
        let _ = std::fs::remove_dir_all(&test_dir);
        std::fs::create_dir_all(&test_dir).unwrap();
        std::env::set_var("HERMES_CRON_TEST_DIR", test_dir.to_str().unwrap());
    ...
    fn test_load_jobs_corrupted_json() {
        let test_dir = std::env::temp_dir().join("hermes_cron_test_corrupted");
        let _ = std::fs::remove_dir_all(&test_dir);
        std::fs::create_dir_all(&test_dir).unwrap();
    ...
    fn test_mark_run_job_not_found() {
        let test_dir = std::env::temp_dir().join("hermes_cron_test_mark_run");
        let _ = std::fs::remove_dir_all(&test_dir);
        std::fs::create_dir_all(&test_dir).unwrap();
        std::env::set_var("HERMES_CRON_TEST_DIR", test_dir.to_str().unwrap());
    ...
    fn test_mark_run_success() {
        let test_dir = std::env::temp_dir().join("hermes_cron_test_mark_success");
        let _ = std::fs::remove_dir_all(&test_dir);
        std::fs::create_dir_all(&test_dir).unwrap();
        std::env::set_var("HERMES_CRON_TEST_DIR", test_dir.to_str().unwrap());
    ...
    fn test_mark_run_completes_repeat_limit() {
        let test_dir = std::env::temp_dir().join("hermes_cron_test_repeat_limit");
        let _ = std::fs::remove_dir_all(&test_dir);
        std::fs::create_dir_all(&test_dir).unwrap();
        std::env::set_var("HERMES_CRON_TEST_DIR", test_dir.to_str().unwrap());
