...
fn load_skill_content(skill_name: &str) -> Result<String> {
    // Try to read skill file from ~/.hermes/skills/
    ...
    if skill_path.exists() {
        return std::fs::read_to_string(&skill_path).map_err(|e| {
            HermesError::new(hermes_core::errors::ErrorCategory::InternalError, format!("Failed to read skill: {e}"))
        });
    }

    // Try bundled skills directory
    let bundled = home.join("hermes-agent").join("skills").join(skill_name);
    if bundled.exists() {
...
fn run_job_script(script_path: &str) -> Result<String> {
    ...
    let _timeout_secs = std::env::var("HERMES_CRON_SCRIPT_TIMEOUT")
        .ok()
    ...
    let sanitized_stdout = hermes_core::redact_sensitive_text(&stdout);
    let sanitized_stderr = hermes_core::redact_sensitive_text(&stderr);
...
mod tests {
    ...
    fn test_redact_sensitive_text() {
        ...
        let output = hermes_core::redact_sensitive_text(input);
