    ...
    pub fn new(config: GatewayConfig) -> Self {
        Self {
            ...
            session_store: Arc::new(SessionStore::new(
                hermez_core::get_hermez_home().join("gateway_sessions"),
    ...
    pub async fn run(&mut self) -> Result<(), String> {
        ...
        let cron_handle = tokio::spawn(async move {
            ...
            loop {
                ...
                let _ = hermez_cron::scheduler::tick_once().await;
            }
        ...
        let health_port = std::env::var("HERMEZ_GATEWAY_HEALTH_PORT")
            .ok()
            ...
            if let Some(platforms_map) = config.get("platforms").and_then(|v| v.as_mapping()) {
                if let Some(feishu_cfg) = platforms_map.get(&serde_yaml::Value::String("feishu".to_string())) {
                    ...
                    if let Some(mode) = feishu_cfg.get("stream_mode").and_then(|v| v.as_str()) {
                        feishu_stream_mode = hermez_core::coerce_bool(mode) || mode == "partial";
                    }
