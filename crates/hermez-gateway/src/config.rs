...
impl GatewayConfig {
    ...
    /// Get the sessions directory, defaulting to ~/.hermez/sessions/.
    pub fn sessions_dir(&self) -> PathBuf {
        self.sessions_dir
            .clone()
            .unwrap_or_else(|| hermez_core::get_hermez_home().join("sessions"))
    }
...
/// 2. ~/.hermez/config.yaml (primary user-facing config)
/// 3. ~/.hermez/gateway.json (legacy)
/// 4. Built-in defaults
...
pub fn save_gateway_config(config: &GatewayConfig, path: Option<&std::path::Path>) -> Result<(), String> {
    let target = path
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|| hermez_core::get_hermez_home().join("config.yaml"));
