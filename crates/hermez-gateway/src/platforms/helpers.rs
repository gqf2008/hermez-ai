...
impl ThreadParticipationTracker {
    ...
    fn _state_path(platform: &str) -> PathBuf {
        hermez_core::get_hermez_home().join(format!("{platform}_threads.json"))
    }
...
fn media_cache_dir(subdir: &str) -> PathBuf {
    let dir = hermez_core::get_hermez_home().join("cache").join(subdir);
...
pub fn cleanup_media_cache(subdir: &str, max_age_hours: u64) -> usize {
    let cache_dir = hermez_core::get_hermez_home().join("cache").join(subdir);
