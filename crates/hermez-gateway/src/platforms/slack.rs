...
const APPROVAL_CHOICE_MAP: &[(&str, &str)] = &[
    ("hermez_approve_once", "once"),
    ("hermez_approve_session", "session"),
    ("hermez_approve_always", "always"),
    ("hermez_deny", "deny"),
];
...
impl SlackAdapter {
    ...
    async fn handle_block_action_payload(&self, payload: &Value) -> Result<(), String> {
        ...
        for action in actions {
            ...
            // Only handle Hermez approval actions.
                ...
                "elements": [
                    {
                        ...
                        "style": "primary",
                        "action_id": "hermez_approve_once",
                        "value": session_key,
                    },
                    {
                        "type": "button",
                        "text": {"type": "plain_text", "text": "Allow Session"},
                        "action_id": "hermez_approve_session",
                        "value": session_key,
                    },
                    {
                        "type": "button",
                        "text": {"type": "plain_text", "text": "Always Allow"},
                        "action_id": "hermez_approve_always",
                        "value": session_key,
                    },
                    {
                        "type": "button",
...
mod tests {
    ...
    fn test_approval_choice_map() {
        assert_eq!(
            APPROVAL_CHOICE_MAP.iter().find(|&&(aid, _)| aid == "hermez_approve_once").map(|&(_, c)| c),
            Some("once")
        );
    }
}
