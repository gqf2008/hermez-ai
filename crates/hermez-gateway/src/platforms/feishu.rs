...
/// Cached Feishu credentials read once from `~/.hermez/config.yaml`.
...
static FEISHU_CONFIG_CACHE: std::sync::LazyLock<(String, String, String, String)> =
    std::sync::LazyLock::new(|| {
        let path = hermez_core::get_hermez_home().join("config.yaml");
...
impl PersistentDedup {
    pub fn new(max_size: usize, ttl_secs: u64) -> Self {
        let path = hermez_core::get_hermez_home().join("feishu_seen_message_ids.json");
...
fn to_boolean(value: Option<&Value>) -> bool {
    match value {
        ...
        Some(Value::String(s)) => hermez_core::coerce_bool(s),
        _ => false,
    }
}
...
impl FeishuAdapter {
    ...
    pub fn new(config: FeishuConfig, approval_registry: crate::runner::ApprovalRegistry) -> Self {
        let dedup_cache_size = std::env::var("HERMEZ_FEISHU_DEDUP_CACHE_SIZE")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(2000);
        Self {
    ...
    pub async fn finalize_stream_reply(&self, chat_id: &str, final_text: &str) -> Result<(), String> {
        ...
        let filtered = hermez_core::strip_think_blocks(final_text);
    ...
    pub async fn send_exec_approval(
        ...
        let btn = |label: &str, action_name: &str, btn_type: &str| -> Value {
            serde_json::json!({
                ...
                "type": btn_type,
                "value": {"hermez_action": action_name, "approval_id": approval_id},
            })
        };
    ...
    async fn download_and_cache_media(
        ...
        let cache_dir = hermez_core::get_hermez_home().join("feishu").join("media");
        ...
        match event_type {
            ...
            "card.action.trigger" => {
                if let Some(action) = event.get("action") {
                    ...
                    // Handle Hermez approval card actions inline
                    if let Some(hermez_action) = action_value
                        .get("hermez_action")
                        .and_then(|v| v.as_str())
                    {
                        let open_id = event
        ...
        match event_type {
            ...
            "card.action.trigger" => {
                if let Some(action) = payload.get("action") {
                    ...
                    // Route non-Hermez card actions to registered handler
            ...
            "im.chat.member.bot.added_v1" => {
                ...
                let _ = self.send_text(chat_id, "Hello! I'm Hermez Agent. How can I help you today?").await;
            }
            "im.chat.member.bot.deleted_v1" => {
                let chat_id = payload
                    .get("event")
