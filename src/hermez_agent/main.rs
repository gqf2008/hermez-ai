//! Hermez Agent — core conversation loop standalone binary.
//!
//! Replaces the Python `hermez-agent` command (run_agent:main).
...
#[derive(Parser)]
#[command(name = "hermez-agent", about = "Hermez Agent core loop", version)]
struct Cli {
    /// Model to use
...
async fn main() -> anyhow::Result<()> {
    ...
    if !cli.quiet {
        println!("Hermez Agent v{}", env!("CARGO_PKG_VERSION"));
