...
//! Mirrors `acp_adapter/server.py` — `HermezACPAgent` class.
...
fn handle_slash_command(cmd: &str, _session: &SessionState) -> Option<String> {
    ...
    match parts[0] {
        ...
        "/version" => Some(format!("hermez-agent {}", env!("CARGO_PKG_VERSION"))),
    ...
    fn send_available_commands_update(&self, _session_id: &str) {
        let commands = vec![
            ...
            AvailableCommand {
                ...
                description: "Show Hermez version".to_string(),
                input: None,
            },
        ];
...
fn resolve_default_model() -> String {
    ...
    if let Ok(model) = std::env::var("HERMEZ_DEFAULT_MODEL") {
        if !model.is_empty() {
            return model;
        }
    }
...
async fn run_agent(
    ...
    let (provider, base_url, api_key) = if let Ok(config) = HermezConfig::load() {
        (
    ...
    let config = AgentConfig {
        ...
        platform: Some("hermez-acp".to_string()),
...
mod tests {
    ...
    fn test_slash_version() {
        ...
        assert!(result.is_some());
        assert!(result.unwrap().contains("hermez-agent"));
    }
