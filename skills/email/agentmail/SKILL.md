---
name: agentmail
description: Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. hermez-agent@agentmail.to).
version: 1.0.0
metadata:
  hermes:
...
Add to `~/.hermez/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `hermez-agent`)
   - The agent gets address: `hermez-agent@agentmail.to`
2. Send an email:
...
```
1. create_inbox (username: "hermez-outreach")
