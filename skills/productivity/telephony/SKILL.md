...
- save provider credentials into `~/.hermez/.env`
...
```bash
SCRIPT="$(find ~/.hermez/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
3. Buy it and save it into `~/.hermez/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `~/.hermez/.env`
