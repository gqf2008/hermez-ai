...
version: 1.0.0
author: arceus77-7, enhanced by Hermez Agent
license: MIT
metadata:
  hermes:
...
Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.hermez/.env` (the skill will prompt for this on first load).
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/hermez-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/hermez-op.sock"
