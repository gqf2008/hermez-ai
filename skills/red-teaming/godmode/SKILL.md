...
exec(open(os.path.expanduser(
    os.path.join(os.environ.get("HERMEZ_HOME", os.path.expanduser("~/.hermez")), "skills/red-teaming/godmode/scripts/load_godmode.py")
)).read())
...
1. **Reads `~/.hermez/config.yaml`** to detect the current model
...
7. **If a strategy works**, locks it in:
   ...
   - Writes prefill messages to `~/.hermez/prefill.json`
...
Then set in `~/.hermez/config.yaml`:
```yaml
agent:
...
exec(open(os.path.join(os.environ.get("HERMEZ_HOME", os.path.expanduser("~/.hermez")), "skills/red-teaming/godmode/scripts/parseltongue.py")).read())
...
```python
# Via execute_code
exec(open(os.path.join(os.environ.get("HERMEZ_HOME", os.path.expanduser("~/.hermez")), "skills/red-teaming/godmode/scripts/godmode_race.py")).read())
...
5. **Hermes models don't need jailbreaking** — nousresearch/hermez-3-* and hermez-4-* are already uncensored. Use them directly for the fastest path.
...
12. **execute_code sandbox has no env vars** — When Hermes runs auto_jailbreak via execute_code, the sandbox doesn't inherit `~/.hermez/.env`. Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.hermez/.env"))`
