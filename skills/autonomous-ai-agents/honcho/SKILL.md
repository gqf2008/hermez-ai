...
Config file: `$HERMEZ_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `hermes honcho setup`. Ensure `memory.provider: honcho` is in `~/.hermez/config.yaml`.
