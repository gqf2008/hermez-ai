...
- Give it a name like "hermez-agent"
- Select scopes:
...
- Give it a title like "hermez-agent-<machine-name>"
...
elif [ -f ~/.hermez/.env ] && grep -q "^GITHUB_TOKEN=" ~/.hermez/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.hermez/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
