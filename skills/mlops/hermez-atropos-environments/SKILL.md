---
name: hermez-atropos-environments
description: Build, test, and debug Hermez Agent RL environments for Atropos training. Covers the HermesAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or fixing RL environments in the hermez-agent repo.
version: 1.1.0
author: Hermez Agent
license: MIT
metadata:
...
# Hermez Agent Atropos Environments

Guide for building RL environments in the hermez-agent repo that integrate with the Atropos training framework.

## Architecture Overview

```
Atropos BaseEnv (atroposlib/envs/base.py)
    └── HermesAgentBaseEnv (environments/hermez_base_env.py)
...
| File | Purpose |
|------|---------|
| `environments/hermez_base_env.py` | Base class with agent loop + tool resolution |
...
The whole point of hermez-agent environments is agentic evaluation:
...
2. **evaluate() must use HermesAgentLoop, not chat_completion** — Single-turn chat_completion has no tools. The whole point of hermez-agent benchmarks is agentic evaluation with tool use.
